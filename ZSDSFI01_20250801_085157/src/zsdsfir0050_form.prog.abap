...
FORM F_CALL_TRANSECTION.
  ...
  READ TABLE GT_REPORT INTO GS_REPORT INDEX LES_ROW_ID-INDEX.
  IF SY-SUBRC = 0.
*    GS_HEADER-CONTN = GS_REPORT-CONTN.
*    GS_HEADER-LIFNR = GS_REPORT-LIFNR.
*    GS_HEADER-RUNNG = GS_REPORT-RUNNG.

*    GV_MODE = GC_MODE-UPDATE.
*    LCL_DATA=>START_PROCESS( ).

    SET PARAMETER ID GC_PARAID-CONTN FIELD GS_REPORT-CONTN.
    SET PARAMETER ID GC_PARAID-LIFNR FIELD GS_REPORT-LIFNR.
    SET PARAMETER ID GC_PARAID-RUNNG FIELD GS_REPORT-RUNNG.

    SUBMIT ZSDSFIR0050 AND RETURN.
  ENDIF.
...
ENDFORM.                    " F_CHECK_BOX
FORM F_SET_REPORT CHANGING LT_DATA TYPE GYT_ZSDSFIT002.

  DATA : LS_DATA LIKE LINE OF LT_DATA.

  LOOP AT LT_DATA INTO LS_DATA.
    MOVE-CORRESPONDING LS_DATA TO GS_REPORT.
