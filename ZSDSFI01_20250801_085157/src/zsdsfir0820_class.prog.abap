...
*
      FROM ACDOCA
      INNER JOIN SKAT  ON SKAT~SAKNR   EQ ACDOCA~RACCT
      LEFT  JOIN CEPCT ON ACDOCA~PRCTR EQ CEPCT~PRCTR
    ...
    SORT LT_TMP BY RACCT PRCTR.

    LOOP AT LT_TMP INTO DATA(LS_DATA).

      MOVE-CORRESPONDING LS_DATA TO LS_TMP.

      IF LV_CHECK NE LS_TMP-RACCT+0(1).
        LV_CHECK = LS_TMP-RACCT+0(1).
      ENDIF.

      GS_RESULT-RACCT = LS_TMP-RACCT.
      GS_RESULT-PRCTR = LS_TMP-PRCTR.
      GS_RESULT-HSL   = LS_TMP-HSL.
      GS_RESULT-TXT50 = LS_TMP-TXT50.
      GS_RESULT-LTEXT = LS_TMP-LTEXT.


      IF GS_RESULT-HSL GE 0.
        GS_RESULT-AMTDB = GS_RESULT-HSL.
        CLEAR GS_RESULT-AMTCD.
      ELSE.
        GS_RESULT-AMTCD = GS_RESULT-HSL.
        CLEAR GS_RESULT-AMTDB.
      ENDIF.

      AT NEW RACCT.
        ADD 1 TO GS_RESULT-RUNGL.
      ENDAT.

      GS_RESULT-RUNGU = LV_CHECK.
