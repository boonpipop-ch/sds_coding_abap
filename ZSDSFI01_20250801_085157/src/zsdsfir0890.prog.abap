...
DATA: BEGIN OF GS_BSID ##NEEDED,
        ...
        REBZJ     TYPE BSEG-REBZJ,  "+CH01
        REBZZ     TYPE BSEG-REBZZ,  "+CH01
        ...
        CK_BOX    TYPE XFELD,
        ANRED     TYPE ANRED,
*<-- Start of Insertion CH01 21.03.2025 (New Fields)
        REF_BUZEI TYPE  BSEG-BUZEI,
        REF_SHKZG TYPE  BSEG-SHKZG,
        REF_ERDAT TYPE  BKPF-CPUDT,
        REF_ERTIM TYPE  BKPF-CPUTM,
*--> End of Insertion CH01 21.03.2025
      END OF GS_BSID,
      ...
      BEGIN OF GS_BSEG ##NEEDED,
        ...
        CPUDT       TYPE BKPF-CPUDT,  "+CH01
        CPUTM       TYPE BKPF-CPUTM,  "+CH01
      END OF GS_BSEG,
...
DATA: GRT_BLART_INV  TYPE RANGE OF BKPF-BLART ##NEEDED,
      ...
      GRT_SPECIAL_CN TYPE RANGE OF BKPF-BLART ##NEEDED.  "+CH01
*----------------------------------------------------------------------*
* FIELD-SYMBOLS
*----------------------------------------------------------------------*
...
FORM GET_DATA .
  ...
  DATA: LC_PROG      TYPE SY-CPROG VALUE 'ZSDSFIR0210'.
    ...
    CASE <L_GENC>-PARAM.
      ...
      WHEN 'DN_DOC_TYPE'.
        ...
        IF R_DN IS NOT INITIAL.
          APPEND LS_BLART TO LRT_BLART.
        ENDIF.
*<-- Start of CH01 17.06.2025 (New GenC Constants)
      WHEN 'SPECIAL_CN_DOC_TYPE'.
        INSERT LS_BLART INTO TABLE GRT_SPECIAL_CN.
*--> End of CH01 17.06.2025
    ENDCASE.
...
*  ENDIF.
  ...
  SELECT A~KUNNR,
         ...
         A~BUDAT,
         A~BLDAT,
         B~CPUDT AS ERDAT, "+CH01
         B~CPUTM AS ERTIM, "+CH01
         A~UMSKZ,
         A~DMBTR,
         A~WAERS,
         A~MWSKZ,
...
*         ON A~GJAHR = C~GJAHR
*        AND A~BELNR = C~BELNR
*<-- Start of Insertion CH01 21.03.2025 (Add BKPF Fields)
...
*<-- Start of Insertion CH01 21.03.2025 (Assign Ref data)
          <L_BSID>-REF_BUZEI  = LS_BSEG-BUZEI.
          <L_BSID>-REF_SHKZG  = LS_BSEG-SHKZG.
          <L_BSID>-REF_ERDAT  = LS_BSEG-CPUDT.
          <L_BSID>-REF_ERTIM  = LS_BSEG-CPUTM.
*--> End of Insertion CH01 21.03.2025
        ENDIF.
*<-- Start of Insertion CH01 21.03.2025 (Check Valid Item)
        IF LS_BSEG-UMSKZ NE 'N' AND
           LS_BSEG-BUZEI NE <L_BSID>-BUZEI.
          LF_VALID = 'X'.
        ENDIF.
*--> End of Insertion CH01 21.03.2025
      ENDLOOP.
...
FORM PRINT_FORM USING UF_ETAX ##PERF_NO_TYPE
  ...
  DATA: LS_ITEM TYPE ZSDSFIS080.   "+CH01
...
*                'FAX' LS_Company_EN-FAX_NUMBER
    ...
    ENDIF.
*<-- Start of Insertion CH01 17.06.2025 (Assign Special Processing CN Text)
    DATA(LF_NEW_REF_START) = ZCL_SDSSD_ETAX_INTERFACE=>GET_NEW_REF_INFO_START( ).
*   Check New Logic activation date and GenC DocType to convert text
    IF NOT ( LF_NEW_REF_START IS NOT INITIAL   AND
             <L_DOC>-ERDAT LT LF_NEW_REF_START ) AND
       GRT_SPECIAL_CN IS NOT INITIAL     AND
       <L_DOC>-BLART IN GRT_SPECIAL_CN.
      LS_FOOTER-REDU_AMT_TXT = 'จำนวนเงิน' ##NO_TEXT.
      LS_FOOTER-REDU_AMT_TXT_EN = 'Amount' ##NO_TEXT.
    ENDIF.
*--> End of Insertion CH01 17.06.2025
...
*    ENDIF.
    ELSE.
      READ TABLE GT_CUSTOMER INTO DATA(LS_CUSTOMER) WITH KEY KUNNR = <L_DOC>-KUNNR
                                                             NATION = LF_SPRAS.
      IF SY-SUBRC = 0.
*        LS_HEADER-CUST_NAME   = |{ LS_CUSTOMER-NAME1 } { LS_CUSTOMER-NAME2 }|. "-CH01
        LS_HEADER-CUST_NAME   = |{ LS_CUSTOMER-NAME1 }{ LS_CUSTOMER-NAME2 }{ LS_CUSTOMER-NAME3 }{ LS_CUSTOMER-NAME4 }|. "+CH01
...
*    WRITE <L_DOC>-FAEDT TO LS_HEADER-DUE_DATE DD/MM/YYYY.
    ...
    READ TABLE GT_BRANCH ASSIGNING FIELD-SYMBOL(<L_BRANCH>) WITH KEY BELNR = <L_DOC>-BELNR
                                                                     GJAHR = <L_DOC>-GJAHR.
    IF SY-SUBRC = 0.
*<-- Start of Insertion CH01 21.03.2025 (Assign Customer Branch)
      PERFORM F_ASSIGN_CUST_BRANCH  USING  <L_BRANCH>-J_1TPBUPL
                                           <L_BRANCH>-DESCRIPTION
                                           LS_CUSTOMER-STCD3
                                  CHANGING LS_HEADER-BRANCH.
*--> End of Insertion CH01 21.03.2025
*<-- Start of Deletion CH01 21.03.2025 (Remove Old code)
...
*        ENDIF.
*      ENDIF.
*--> End of Deletion CH01 25.03.2025
    ENDIF.
    ...
    LOOP AT GT_BSEG INTO DATA(LS_BSEG) WHERE BELNR = <L_DOC>-BELNR ##INTO_OK
      ...
      IF LS_BSEG-MWART = 'A'.
        LS_FOOTER-TAX = LS_FOOTER-TAX + LS_BSEG-WRBTR.
        CONTINUE.
      ENDIF.
      ...
      ELSEIF LS_BSEG-KOART = 'S' AND LS_BSEG-SHKZG = 'H' AND LS_BSEG-HKONT CP '1*'.
        LF_ITEM = 'X'.
*<-- Start of Insertion 420000682 30.06.2025 (New Item case)
      ELSEIF LS_BSEG-KOART = 'S' AND LS_BSEG-HKONT CP '5*'.
        LF_ITEM = 'X'.
*--> End of Insertion 420000682 30.06.2025
      ELSE.
...
*            LF_ITEM = 'X'.
          ...
          ELSEIF ( <L_DOC>-BLART IN GRT_BLART_CN AND <L_DOC>-UMSKZ = 'A' "AND <L_DOC>-AUGBL = <L_DOC>-BELNR
              ...
              OR ( <L_DOC>-BLART IN GRT_BLART_CN AND <L_DOC>-UMSKZ = 'S' "AND <L_DOC>-AUGBL = <L_DOC>-BELNR
                 AND LS_BSEG-KOART = 'S' AND LS_BSEG-MWART = 'A' AND LS_BSEG-MWSKZ = 'O7' ) .
            LF_ITEM = 'M'.
*<-- Start of Insertion 30.05.2025 (New Item Condition)
          ELSEIF <L_DOC>-BLART IN GRT_BLART_CN AND
                 LS_BSEG-KOART = 'D' AND
                 LS_BSEG-UMSKZ = 'S'.
            LF_ITEM = 'X'.
*--> End of Insertion 30.05.2025
*<-- Start of Insertion 16.05.2025 (New Item Condition)
          ELSEIF <L_DOC>-BLART IN GRT_BLART_CN AND
                 LS_BSEG-UMSKZ = 'A' AND
                 LS_BSEG-KOART = 'D' .
            LF_ITEM = 'X'.
*--> End of Insertion 16.05.2025
          ENDIF.
        ENDIF.
      ENDIF.
...
*<-- Start of Insertion CH01 21.03.2025 (Get Reference Info)
    DATA: LS_REFINPUT TYPE ZCL_SDSSD_ETAX_INTERFACE=>TS_REFINPUT,
          LS_REFINFO  TYPE ZCL_SDSSD_ETAX_INTERFACE=>TS_REFINFO.

    CLEAR LS_REFINPUT.
    LS_REFINPUT-BUKRS  = P_BUKRS.
    LS_REFINPUT-BELNR  = <L_DOC>-BELNR.
    LS_REFINPUT-GJAHR  = <L_DOC>-GJAHR.
    LS_REFINPUT-SGTXT  = <L_DOC>-SGTXT.
    LS_REFINPUT-REBZG  = <L_DOC>-REBZG.
    LS_REFINPUT-REBZJ  = <L_DOC>-REBZJ.
    LS_REFINPUT-REBZZ  = <L_DOC>-REBZZ.
    LS_REFINPUT-UMSKZ  = <L_DOC>-UMSKZ.
    LS_REFINPUT-ZUONR  = <L_DOC>-ZUONR.
    LS_REFINPUT-KUNNR  = <L_DOC>-KUNNR.
    LS_REFINPUT-AMOUNT = LS_FOOTER-TOTAL.
    IF <L_DOC>-REF_BUZEI IS NOT INITIAL.
      LS_REFINPUT-BUZEI  = <L_DOC>-REF_BUZEI.
      LS_REFINPUT-ERDAT  = <L_DOC>-REF_ERDAT.
      LS_REFINPUT-ERTIM  = <L_DOC>-REF_ERTIM.
      LS_REFINPUT-SHKZG  = <L_DOC>-REF_SHKZG.
    ELSE.
      LS_REFINPUT-BUZEI  = <L_DOC>-BUZEI.
      LS_REFINPUT-ERDAT  = <L_DOC>-ERDAT.
      LS_REFINPUT-ERTIM  = <L_DOC>-ERTIM.
      LS_REFINPUT-SHKZG  = <L_DOC>-SHKZG.
    ENDIF.

    CALL METHOD ZCL_SDSSD_ETAX_INTERFACE=>GET_REF_INFO(
      EXPORTING
        IS_INPUT   = LS_REFINPUT
      IMPORTING
        ES_REFINFO = LS_REFINFO ).

    IF LS_REFINFO-SPECIAL_REF EQ 'X'.
      CLEAR LS_ITEM.
      LS_ITEM-SGTXT = LS_REFINFO-ITEM_TEXT.
      MODIFY LT_ITEM FROM LS_ITEM TRANSPORTING SGTXT
                          WHERE SGTXT NE LS_ITEM-SGTXT.
    ENDIF.
    LS_HEADER-SPECIAL_REF = LS_REFINFO-SPECIAL_REF.
    LS_HEADER-INV_NO = LS_REFINFO-BELNR.
    WRITE LS_REFINFO-ORG_AMT TO LS_FOOTER-TAX_AMT CURRENCY <L_DOC>-WAERS.
    CONDENSE LS_FOOTER-TAX_AMT NO-GAPS.
    WRITE LS_REFINFO-CORRCT_AMT TO LS_FOOTER-TOT_AMT CURRENCY <L_DOC>-WAERS.
    CONDENSE LS_FOOTER-TOT_AMT NO-GAPS.
    WRITE LS_REFINFO-DIFF_AMT TO LS_FOOTER-REDU_AMT CURRENCY <L_DOC>-WAERS.
    CONDENSE LS_FOOTER-REDU_AMT NO-GAPS.
    IF R_CN IS NOT INITIAL OR R_DN IS NOT INITIAL.
      READ TABLE GT_T247 ASSIGNING <L_T247> WITH KEY MNR = LS_REFINFO-BUDAT+4(2).
      IF SY-SUBRC = 0.
        LS_HEADER-INV_DATE = |{ LS_REFINFO-BUDAT+6(2) }-{ <L_T247>-KTX }-{ LS_REFINFO-BUDAT(4) }|.
      ENDIF.
    ENDIF.
*   Update Branch
    IF LS_REFINFO-J_1TPBUPL IS NOT INITIAL.
      PERFORM F_ASSIGN_CUST_BRANCH  USING  LS_REFINFO-J_1TPBUPL
                                           LS_REFINFO-DESCRIPTION
                                           LS_CUSTOMER-STCD3
                                  CHANGING LS_HEADER-BRANCH.
    ENDIF.
*--> End of Insertion CH01 21.03.2025
...
*      CLEAR LF_BUDAT.
*    ENDIF.
*--> End of Deletion CH01 21.03.2025
...
*<-- Start of Insertion CH01 21.03.2025 (New Subroutines)
*----------------------------------------------------------------------*
*  Form F_ASSIGN_CUST_BRANCH
*----------------------------------------------------------------------*
*  Assign Customer Branch
*----------------------------------------------------------------------*
FORM F_ASSIGN_CUST_BRANCH  USING  UF_J_1TPBUPL  TYPE FITHA_PBUPL_D_T-J_1TPBUPL
                                  UF_DESCRIPTION TYPE FITHA_PBUPL_D_T-DESCRIPTION
                                  UF_CUST_TAX_ID TYPE KNA1-STCD3
                         CHANGING CF_BRANCH TYPE ZSDSFIS079-BRANCH.

* Initialize Output
  CLEAR: CF_BRANCH.

  IF UF_J_1TPBUPL = '00000' OR UF_J_1TPBUPL = 'NVAT'.
    SPLIT UF_DESCRIPTION AT '/' INTO DATA(L_EN) DATA(L_TH).
    CASE P_SPRAS.
      WHEN 'E'.
        CF_BRANCH = L_EN.
      WHEN '2'.
    ...
    ENDCASE.
  ELSE.
    CF_BRANCH = UF_DESCRIPTION.
  ENDIF.

  IF UF_CUST_TAX_ID = '0000000000000' OR
     UF_CUST_TAX_ID IS INITIAL.
    CLEAR: CF_BRANCH.
  ELSE.
    IF UF_J_1TPBUPL = 'NVAT' .
      CLEAR CF_BRANCH .
    ELSE.
      IF UF_J_1TPBUPL <> '00000'.

        CASE P_SPRAS.
          WHEN 'E'.
            "Text-T02 :  Branch no.
            CONCATENATE TEXT-T02 UF_J_1TPBUPL
                   INTO CF_BRANCH SEPARATED BY SPACE.
          WHEN '2'.
            "Text-T01: สาขาที่  :
            CONCATENATE TEXT-T01 UF_J_1TPBUPL
                   INTO CF_BRANCH SEPARATED BY SPACE.
          WHEN OTHERS.
        ENDCASE.
      ENDIF.
