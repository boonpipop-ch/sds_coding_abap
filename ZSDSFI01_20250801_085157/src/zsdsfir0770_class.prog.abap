...
CLASS LCL_DATA IMPLEMENTATION.
  ...
  METHOD START_PROCESS.
    ...
    SELECT ACDOCA~*,
           SKAT~TXT50,
           ( KNA1~NAME1 && ' ' && KNA1~NAME2 && ' ' && KNA1~NAME3 && ' ' && KNA1~NAME4 ) AS NAME_ALL

    FROM ACDOCA
    INNER JOIN SKAT ON ACDOCA~RACCT EQ SKAT~SAKNR
                   AND SKAT~KTOPL   EQ 'RCOA'
                   AND SKAT~SPRAS   EQ  @SY-LANGU
    LEFT JOIN KNA1 ON ACDOCA~KUNNR EQ KNA1~KUNNR
    WHERE
             ACDOCA~RBUKRS           IN @S_RBUKRS AND
             ACDOCA~GJAHR            IN @S_GJAHR  AND
             ACDOCA~BELNR            IN @S_BELNR  AND
             ACDOCA~BUDAT            IN @S_BUDAT  AND
             ACDOCA~BLDAT            IN @S_BLDAT  AND
             ACDOCA~PS_PSP_PNR       IN @S_PSPNR  AND
             ACDOCA~PS_POSID         IN @S_POSID  AND
             ACDOCA~PRODH_PA         IN @S_PRODH  AND
             ACDOCA~VKBUR_PA         IN @S_VKBUR  AND
             ACDOCA~VKGRP_PA         IN @S_VKGRP  AND
             ACDOCA~PRCTR            IN @S_PRCTR  AND
             ACDOCA~RCNTR            IN @S_RCNTR  AND
             ACDOCA~AUFNR            IN @S_AUFNR  AND
             ACDOCA~RLDNR            IN @S_RLDNR  AND
             ACDOCA~KUNNR            IN @S_KUNNR
