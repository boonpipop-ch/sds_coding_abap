  ...
  METHOD START_PROCESS.
    ...
    WHERE ANLA~BUKRS IN @S_BUKRS[]
      ...
      AND ANLZ~STORT IN @S_STORT[]
       AND ZSDSFIT055~ERDAT IN @S_ERDAT[]
...
*    CONCATENATE: REP01  TEXT-B01 INTO REP01.
  ENDMETHOD.
  METHOD DELETE_DATA.
    DATA : LT_ZSDSFIT057 TYPE TABLE OF ZSDSFIT057.
    DATA : LS_TMP LIKE LINE OF LT_ZSDSFIT057.

    DATA : LT_ZSDSFIT058 TYPE TABLE OF ZSDSFIT058.


    DATA(LV_CONFIRM) = POPUP_CONFIRM( TEXT-110 ).
    IF LV_CONFIRM EQ GC_ONE.
      SELECT *
        FROM ZSDSFIT050
        INTO CORRESPONDING FIELDS OF TABLE LT_ZSDSFIT057.

        CONCATENATE SY-DATUM SY-UZEIT INTO LS_TMP-VERSION.

        MODIFY LT_ZSDSFIT057 FROM LS_TMP TRANSPORTING VERSION
                                       WHERE VERSION IS INITIAL.

        MODIFY ZSDSFIT057 FROM TABLE LT_ZSDSFIT057.


      SELECT *
        FROM ZSDSFIT055
        INTO CORRESPONDING FIELDS OF TABLE LT_ZSDSFIT058.

        MODIFY ZSDSFIT058 FROM TABLE LT_ZSDSFIT058.


      IF SY-SUBRC EQ 0.
        DELETE FROM ZSDSFIT050.
        DELETE FROM ZSDSFIT055.
        MESSAGE S998 WITH text-s01.
      ELSE.
        MESSAGE S998 WITH text-e01 DISPLAY LIKE 'E'.
      ENDIF.
