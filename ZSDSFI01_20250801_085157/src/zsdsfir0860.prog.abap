...
*-----------------------------------------------------------------------
REPORT ZSDSFIR0860 MESSAGE-ID ZSDSFI01.
...
SELECTION-SCREEN BEGIN OF BLOCK BLOCK2 WITH FRAME TITLE TEXT-002.
  PARAMETERS : P_BUDAT TYPE BKPF-BUDAT OBLIGATORY DEFAULT SY-DATUM,
               P_BLDAT TYPE BKPF-BLDAT NO-DISPLAY,
               P_XBLNR TYPE BKPF-XBLNR NO-DISPLAY,
               P_BKTXT TYPE BKPF-BKTXT NO-DISPLAY.
...
FORM F_GET_DATA.
  ...
  IF GT_RESULT IS NOT INITIAL.
    SELECT EKET~EBELN
      ...
      WHERE EKPO~EBELN EQ GT_RESULT-EBELN
    ...
    IF GT_DETAIL IS NOT INITIAL.
      ...
      LOOP AT LT_MSEG INTO LS_MSEG.
        ...
        IF LS_MSEG-SHKZG EQ 'H'.
          ...
          LS_MSEG_SUM-MENGE = LS_MSEG_SUM-MENGE * -1.
        ENDIF.
      ...
      LOOP AT LT_EKBE INTO LS_EKBE.
        MOVE-CORRESPONDING LS_EKBE TO LS_SUM.
        COLLECT LS_SUM INTO LT_SUM.
        CLEAR : LS_EKBE,LS_SUM.
      ENDLOOP.
    ...
    LOOP AT GT_RESULT ASSIGNING <LFS_RESULT>.
      ...
      IF SY-SUBRC EQ 0.
        <LFS_RESULT>-XBLNR = LS_MKPF-XBLNR.
        <LFS_RESULT>-BLDAT = LS_MKPF-BLDAT.
      ...
      CLEAR : LV_CHK.
      LOOP AT GT_DETAIL INTO GS_DETAIL WHERE EBELN EQ <LFS_RESULT>-EBELN.
        LV_TABIX1 = SY-TABIX.
        ...
        IF SY-SUBRC = 0.
          LV_AMT = GS_DETAIL-WEMNG - LS_SUM-MENGE.
        ELSE.
          LV_AMT = GS_DETAIL-WEMNG.
        ENDIF.
        ...
        IF LV_AMT NE 0.
          ...
          IF SY-SUBRC EQ 0.
            ...
            GS_DETAIL-MENGE = LS_MSEG_TMP-MENGE.

            IF GS_DETAIL-MENGE LE 0.
              GS_DETAIL-MENGE = 1.
            ENDIF.

            GS_DETAIL-NETPR = GS_ZSFI_POST_MIRO-DMBTR.
            MODIFY GT_DETAIL FROM GS_DETAIL INDEX LV_TABIX1
                                     TRANSPORTING MENGE NETPR.
          ELSE.
            GS_ZSFI_POST_MIRO-DMBTR = LV_AMT * GS_DETAIL-NETPR.
          ENDIF.
          ADD GS_ZSFI_POST_MIRO-DMBTR TO <LFS_RESULT>-DMBTR.
          LV_CHK = 'X'.
        ELSE.
          READ TABLE LT_MSEG_SUM INTO LS_MSEG_SUM
          WITH KEY EBELN = GS_DETAIL-EBELN
                   EBELP = GS_DETAIL-EBELP.
          IF SY-SUBRC EQ 0.
            GS_ZSFI_POST_MIRO-DMBTR = ( GS_DETAIL-WEMNG * GS_DETAIL-NETPR ) - LS_MSEG_SUM-DMBTR.
            IF GS_ZSFI_POST_MIRO-DMBTR NE 0.
              READ TABLE LT_MSEG_TMP INTO LS_MSEG_TMP
              WITH KEY EBELN = GS_DETAIL-EBELN
                       EBELP = GS_DETAIL-EBELP.
              IF SY-SUBRC EQ 0.
              ...
              ENDIF.
            ELSE.
              DELETE GT_DETAIL INDEX LV_TABIX1.
            ENDIF.
          ELSE.
            DELETE GT_DETAIL INDEX LV_TABIX1.
          ENDIF.
        ...
        CLEAR : GS_ZSFI_POST_MIRO,GS_DETAIL,GS_ZSFI_POST_MIRO.
      ENDLOOP.
...
FORM F_CALL_TRANSECTION.
  ...
  DATA: P_KVERM TYPE LFB1-KVERM.

  DATA : P_MWSBP TYPE VBAP-MWSBP.
  ...
  IF SY-SUBRC = 0.
    ...
    P_BLDAT = GS_RESULT-BLDAT.
    P_XBLNR = GS_RESULT-XBLNR.
    P_BKTXT = GS_RESULT-BKTXT.
    P_MWSBP = GS_RESULT-MWSBP.
    P_KVERM = GS_RESULT-KVERM.

    LOOP AT GT_DETAIL INTO GS_DETAIL WHERE EBELN EQ GS_RESULT-EBELN.
      CLEAR S_EBELN.
      S_EBELN-SIGN   = 'I'.
      S_EBELN-OPTION = 'EQ'.
      S_EBELN-LOW    = GS_DETAIL-EBELN.
      APPEND S_EBELN.

      CLEAR S_EBELP.
      S_EBELP-SIGN   = 'I'.
      S_EBELP-OPTION = 'EQ'.
      S_EBELP-LOW    = GS_DETAIL-EBELP.
      APPEND S_EBELP.
      CLEAR : GS_DETAIL.
    ENDLOOP.

    SUBMIT ZSDSFIR0850 USING SELECTION-SCREEN 1000
                           WITH P_BUKRS EQ P_BUKRS
                           WITH S_EBELN IN S_EBELN
                           WITH S_EBELP IN S_EBELP
                           WITH P_BUDAT EQ P_BUDAT
                           WITH P_BLDAT EQ P_BLDAT
                           WITH P_XBLNR EQ P_XBLNR
                           WITH P_BKTXT EQ P_BKTXT
                           WITH P_MWSBP EQ P_MWSBP
                           WITH P_KVERM EQ P_KVERM
                           AND RETURN.
...
FORM F_SAVE_FROM_HEADER.
  ...
  LOOP AT GT_RESULT INTO GS_RESULT WHERE CHECK EQ 'X'.
    LOOP AT GT_DETAIL INTO GS_DETAIL WHERE EBELN EQ GS_RESULT-EBELN.
      LV_AMT = GS_DETAIL-MENGE." - gs_detail-wemng.
      GS_ZSFI_POST_MIRO-EBELN = GS_DETAIL-EBELN.
      GS_ZSFI_POST_MIRO-EBELP = GS_DETAIL-EBELP.
      GS_ZSFI_POST_MIRO-MENGE = LV_AMT.
      GS_ZSFI_POST_MIRO-DMBTR = GS_DETAIL-NETPR.
      IF GS_RESULT-KVERM EQ 'VAT'.
        GS_ZSFI_POST_MIRO-FLAGV =  'X'.
        GS_ZSFI_POST_MIRO-MWSBP = GS_RESULT-MWSBP.
      ...
      CLEAR : GS_ZSFI_POST_MIRO,GS_DETAIL.
    ENDLOOP.
    CLEAR : GS_RESULT.
  ENDLOOP.
...
*----------------------------------------------------------------------*
FORM F_REFRESH_DATA.
  CLEAR : GT_RESULT,GS_RESULT,GT_DETAIL,GS_DETAIL,GT_SAVE,GS_SAVE,GT_ZSFI_POST_MIRO,GS_ZSFI_POST_MIRO,P_LIFNR.
