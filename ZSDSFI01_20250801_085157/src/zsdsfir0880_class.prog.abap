...
CLASS LCL_DATA DEFINITION.
  PUBLIC SECTION.
    ...
    CLASS-METHODS :
      ...
      SET_ALV_GRID,
      HTML_TOP_OF_PAGE,
      MODIFY_URL CHANGING C_URL TYPE STRING.
    CLASS-DATA :
...
CLASS LCL_DATA IMPLEMENTATION.
  ...
  METHOD START_PROCESS.
    DATA : LCL_API TYPE REF TO ZCL_SDSCA_CAL_API.
    ...
    DATA: BEGIN OF LS_BODY,
          ...
          END OF LS_BODY.
    DATA : LT_BODY LIKE TABLE OF LS_BODY.

    DATA: BEGIN OF LS_RETURN,
            _Value         TYPE STRING,
            _Formatters    TYPE STRING,
            _Content_Types TYPE STRING,
            _Declared_Type TYPE STRING,
            _Status_Code   TYPE STRING,
          END OF LS_RETURN.
    ...
    LV_METHOD    = 'GET'.
    LV_URL       = LCL_API->GET_API_END_POINT( 'GP_REPORT' ).
    MODIFY_URL( CHANGING C_URL = LV_URL ).
    LT_HEADER    = LCL_API->GET_API_HEADER( ).
*    LV_BODY_TEXT = LCL_API->GET_API_DETAIL( I_DATA_TAB = LT_BODY ).
*    LV_LEN       = LCL_API->GET_API_LEN( LV_BODY_TEXT ).
    ...
    CALL METHOD ZCL_SDSCA_CAL_API=>CALL_API
      ...
      IMPORTING
        ...
        E_STATUS           = LV_STATUS.
    IF LS_RETURN-_Status_Code EQ '200'.
      MESSAGE S998 WITH LS_RETURN-_Value.
    ELSE.
