...
*&---------------------------------------------------------------------*
  ...
  CASE p_ucomm.
    WHEN '&IC1'.
      PERFORM f_call_transection USING p_selfld.
    WHEN 'ALL'.
...
*  <--  p2        text
*----------------------------------------------------------------------*
FORM f_call_transection USING rs_selfield TYPE slis_selfield..
  DATA: le_row     TYPE i,
        le_value   TYPE c,
        le_col     TYPE i,
  ...
  IF rs_selfield-fieldname EQ 'VBELN'.
        ...
        ENDIF.
  ELSEIF rs_selfield-fieldname EQ 'LIEF_NR'.
       READ TABLE GT_ITAB INTO WA_ITAB INDEX rs_selfield-tabindex..
        IF SY-SUBRC = 0.
          SET PARAMETER ID 'VL'  FIELD WA_ITAB-LIEF_NR.
