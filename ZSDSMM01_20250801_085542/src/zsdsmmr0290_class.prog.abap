...
CLASS LCL_DATA DEFINITION.
  PUBLIC SECTION.
    ...
    CLASS-METHODS :
      ...
      SET_ALV_GRID,
      HTML_TOP_OF_PAGE,
      UPDATE_ZSDSMMT006 IMPORTING I_WEBNO TYPE ZSDSDE_WEB_NUMBER
                                  I_PR    TYPE BANFN.
...
CLASS LCL_DATA IMPLEMENTATION.
    ...
    LOOP AT GT_RESULT ASSIGNING <LFS_RESULT>.
      ...
      CALL FUNCTION 'Z_SDSMM_CREATE_PR'
        EXPORTING
          ...
          I_WEBNO            = <LFS_RESULT>-WEBNO
          I_SEND_MAIL        = ABAP_TRUE
        IMPORTING
          E_OUTPUT           = LV_OUTPUT
          E_MESSAGE          = LV_MESSAGE.
      IF LV_OUTPUT IS NOT INITIAL.
        UPDATE_ZSDSMMT006( I_WEBNO = <LFS_RESULT>-WEBNO
                           I_PR    = LV_OUTPUT ).
        <LFS_RESULT>-PRNO  = LV_OUTPUT.
        <LFS_RESULT>-MESSG = LV_MESSAGE.
      ELSE.
        <LFS_RESULT>-MESSG = LV_MESSAGE.
      ENDIF.
    ENDLOOP.
  ...
  ENDMETHOD.
  METHOD UPDATE_ZSDSMMT006.
    DATA : LS_ZSDSMMT006 TYPE ZSDSMMT006.

    LS_ZSDSMMT006-BANFN = I_PR.
    LS_ZSDSMMT006-WEBNO = I_WEBNO.
    LS_ZSDSMMT006-FLAGD = SPACE.
    ...
    DELETE FROM ZSDSMMT006 WHERE WEBNO EQ I_WEBNO
                             AND BANFN EQ SPACE.
