FUNCTION Z_SDSMM_PHYSICAL_COUNT_FG.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     VALUE(I_INPUT) TYPE  ZSDSMMT024 OPTIONAL
*"     VALUE(I_MODE) TYPE  CHAR10 OPTIONAL
*"  EXPORTING
*"     VALUE(E_MESTYPE) TYPE  CHAR1
*"     VALUE(E_MESSAGE) TYPE  CHAR255
*"  TABLES
*"      T_SEARCH STRUCTURE  ZSDSFIS142
*"  CHANGING
...
*"----------------------------------------------------------------------
  ...
  ELSEIF I_MODE EQ GC_CON-GET_DATA.
    T_OUTPUT = LCL_DATA=>GET_DATA_FG( I_INPUT ).
  ELSEIF I_MODE EQ GC_CON-INSERT_DATA.
    LCL_DATA=>UPDATA_PHYSICAL_FG( EXPORTING I_DATA    = I_INPUT
