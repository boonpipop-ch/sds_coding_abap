...
CLASS LCL_DATA IMPLEMENTATION.
  ...
  METHOD START_PROCESS.
    SELECT A~RSNUM,
           ...
           ZSDSMMT028~SENDS,
           CASE H~SMBLN
                WHEN @SPACE THEN I~MBLNR
                ELSE H~SMBLN
           END AS MBLNR,
           CASE H~SJAHR
                WHEN '0000' THEN I~MJAHR
                ELSE H~SJAHR
           END AS MJAHR,
           CASE H~SMBLP
                WHEN '0000' THEN I~ZEILE
                ELSE H~SMBLP
           END AS ZEILE,
           G~LGOBE
          FROM ZSDSMMT028
          ...
          LEFT JOIN MSEG AS H ON A~RSNUM EQ H~RSNUM AND
                                 ...
                                 H~SHKZG EQ 'H'
          LEFT JOIN MSEG AS I ON I~SMBLN EQ H~MBLNR AND
                                 I~SJAHR EQ H~MJAHR AND
                                 I~SMBLP EQ H~ZEILE
...
*  CALL METHOD document->new_line.
  ...
  METHOD SEND.
    ...
    DELETE ADJACENT DUPLICATES FROM LT_TMP COMPARING RSNUM RSPOS.

    LOOP AT LT_DOC INTO LS_DOC.
      LOOP AT LT_TMP ASSIGNING FIELD-SYMBOL(<LFS_DATA>) WHERE RSNUM EQ LS_DOC-RSNUM.
...
*            MESSAGE S000 WITH TEXT-E01 DISPLAY LIKE 'E'.
          ...
          MODIFY GT_RESULT FROM <LFS_DATA> TRANSPORTING SENDS
