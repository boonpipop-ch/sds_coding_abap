...
CLASS LCL_DATA DEFINITION.
  PUBLIC SECTION.
    CLASS-METHODS :
      GET_BILLING_DETAIL IMPORTING I_VBELN  TYPE VBRK-VBELN
                                   I_KUNNR  TYPE VBRK-KUNRG
      ...
      CALL_SMART_FORM IMPORTING I_DATA   TYPE LBBIL_INVOICE
                      RETURNING VALUE(R) TYPE ZSDSCAS019,
...
CLASS LCL_DATA IMPLEMENTATION.
  METHOD GET_BILLING_DETAIL.
    ...
    DATA(LS_PRINT_DATA_TO_READ) = SET_PRINT_DATA_TO_READ( 'ZSDSFI003' ).
...
*       job_output_options =
    ...
    R = COVERT_OTF( LS_JOB_INFO-OTFDATA ).
  ENDMETHOD.
  METHOD COVERT_OTF.
...
CONSTANTS: GC_MATGRP_INS          TYPE MARA-MATKL VALUE 'IN-INSTAL'.
*----------------------------------------------------------------------*
* INTERNAL TABLES
*----------------------------------------------------------------------*
...
FORM F_PRINT USING UF_RETCODE     TYPE SY-SUBRC
  ...
  DATA: LR_VTWEG TYPE RANGE OF VBRK-VTWEG,
        LR_VKGRP TYPE RANGE OF VBRP-VKGRP.

  DATA: PM_ETAXM TYPE VBCO7-ALLEF.
  ...
  CONSTANTS : BEGIN OF LC_CON,
                I_REPID TYPE CHAR20 VALUE 'ZSDSSDR0420',
                I_PARAM TYPE CHAR20 VALUE 'ETAX_CHANNAL',
              END OF LC_CON.

  CONSTANTS : BEGIN OF LC_CON1,
                I_REPID TYPE CHAR20 VALUE 'ZSDSSDR0420',
                I_PARAM TYPE CHAR20 VALUE 'ETAX_SALES_GROUP',
              END OF LC_CON1.
  ...
  LCL_UTIL->GET_CONFIG_001( EXPORTING I_REPID              = LC_CON-I_REPID
                             ...
                             CHANGING CR_RETURN            = LR_VTWEG
                           ).

  LCL_UTIL->GET_CONFIG_001( EXPORTING I_REPID              = LC_CON1-I_REPID
                                      I_PARAM              = LC_CON1-I_PARAM
                             CHANGING CR_RETURN            = LR_VKGRP
                           ).

  CONCATENATE SY-UNAME '_' SY-DATUM INTO LV_MEMORY.
  IMPORT PM_ETAXM TO PM_ETAXM FROM MEMORY ID LV_MEMORY.
...
*     error handling
  ...
  IF PM_ETAXM EQ ABAP_TRUE.
    DATA : LV_VKGRP TYPE VBAK-VKGRP.

    CONSTANTS : BEGIN OF LC_CON3,
                  I_REPID TYPE CHAR20 VALUE 'ZSDSSDR0420',
                  I_PARAM TYPE CHAR20 VALUE 'EX_EMAIL_ETAX',
                END OF LC_CON3.

    DATA : LR_EMAIL     TYPE RANGE OF ZSDSFIC001-E_MAIL.

    LCL_UTIL->GET_CONFIG_001( EXPORTING I_REPID   = LC_CON3-I_REPID
                                        I_PARAM   = LC_CON3-I_PARAM
                               CHANGING CR_RETURN = LR_EMAIL ).

    SELECT SINGLE KUNNR,
                  E_MAIL
      FROM ZSDSFIC001
      INTO @DATA(LS_ZTE_CUST_EMAIL)
      WHERE KUNNR  EQ @LS_BIL_INVOICE-HD_GEN-PAYER
        AND E_MAIL IN @LR_EMAIL.
    IF LS_ZTE_CUST_EMAIL-E_MAIL IS NOT INITIAL.
      IF LS_BIL_INVOICE-HD_GEN-TERMS_PAYM = 'C000' OR
         LS_BIL_INVOICE-HD_GEN-TERMS_PAYM = 'C999' OR
         LS_BIL_INVOICE-HD_GEN-TERMS_PAYM = 'C007' .
        LV_ETAX = ABAP_TRUE.
      ELSE.
        CLEAR : LV_ETAX.
      ENDIF.
...
FORM  F_PREPARE_FORM_ITEMS  USING  US_BIL_INVOICE  TYPE LBBIL_INVOICE
  ...
  IF LF_FOUND_ZH16  EQ ABAP_TRUE. "text ZH16 found
    ...
    LOOP AT LT_VBRP ASSIGNING <L_VBRP>.
      ...
      IF <L_VBRP>-MATNR IN GRT_MAT_NOT_SHOW_DESC AND
         <L_VBRP>-NETWR = 0.
        CONTINUE.
      ENDIF.
  ...
  ELSE. "Text ZH16 not found
    ...
    LOOP AT LT_VBRP ASSIGNING <L_VBRP> WHERE UEPOS IS INITIAL.
      ...
      LOOP AT LT_VBRP ASSIGNING <L_BOM> WHERE UEPOS EQ <L_VBRP>-POSNR. "#EC CI_NESTED
        ...
        IF <L_BOM>-MATNR IN GRT_MAT_NOT_SHOW_DESC AND
           <L_BOM>-NETWR = 0.
          CONTINUE.
        ENDIF.
...
FORM F_GENERATE_PDF  USING  US_BIL_INVOICE  TYPE LBBIL_INVOICE
  ...
  IF NOT LF_BIN_FILE IS INITIAL.
    ...
    IF UF_MHA EQ 'MHA'.
      PERFORM F_SEND_EMAIL_MHA USING US_BIL_INVOICE-HD_GEN-BIL_NUMBER
                                     LF_BIN_FILESIZE
                                     LF_BIN_FILE.
    ELSEIF UF_MHA EQ 'MTD' or
           UF_MHA EQ 'TCR'.
      PERFORM F_SEND_EMAIL_MTD USING US_BIL_INVOICE-HD_GEN-BIL_NUMBER
                                     US_BIL_INVOICE-HD_GEN-SOLD_TO_PARTY
                                     LF_BIN_FILESIZE
                                     LF_BIN_FILE.
    ENDIF.
...
FORM F_SET_PRINT_PARAM  USING     US_ADDR_KEY       TYPE ADDR_KEY
  ...
  CONSTANTS : BEGIN OF LC_CON,
                ...
                ZPRINT TYPE C LENGTH 9 VALUE 'ZSDSSD061',
              END OF LC_CON.
  ...
  IF CF_RETCODE = 0.
    IF US_PROC_SCREEN  EQ SPACE.
      ...
      IF SY-TCODE EQ LC_CON-ZVF31 OR
         SY-TCODE EQ LC_CON-VF31  OR
         SY-TCODE EQ LC_CON-ZPRINT.
        CS_CONTROL_PARAM-NO_DIALOG   = ABAP_TRUE.
      ENDIF.
...
*&      --> LF_BIN_FILE
*&---------------------------------------------------------------------*
FORM F_SEND_EMAIL_MHA  USING LV_VBELN        TYPE VBRK-VBELN
  ...
  DATA : LR_CC  LIKE RANGE OF LS_EMAIL.
  DATA : LR_REC LIKE RANGE OF LS_EMAIL.

  CONSTANTS : BEGIN OF LC_CON,
                I_REPID TYPE CHAR20 VALUE 'EMAIL_GROUP',
                I_PARAM TYPE CHAR20 VALUE 'LOGISTIC',
              END OF LC_CON.

  CONSTANTS : BEGIN OF LC_CON1,
                I_REPID TYPE CHAR20 VALUE 'MHA_EMAIL',
                I_PARAM TYPE CHAR20 VALUE 'SONY',
              END OF LC_CON1.
  ...
  CONCATENATE : 'MHA Billing' LV_DO '/' LV_VBELN INTO LV_SUBJECT SEPARATED BY SPACE.

  LS_ATTACH_FILE-GROUP    = 1.
  LS_ATTACH_FILE-FILENAME = LV_VBELN.
  LS_ATTACH_FILE-LEN      = LV_BIN_FILESIZE.

  LCL_UTIL->GET_CONFIG_001( EXPORTING I_REPID   = LC_CON-I_REPID
                                      I_PARAM   = LC_CON-I_PARAM
                             CHANGING CR_RETURN = LR_CC
                           ).

  LCL_UTIL->GET_CONFIG_001( EXPORTING I_REPID   = LC_CON1-I_REPID
                                      I_PARAM   = LC_CON1-I_PARAM
                             CHANGING CR_RETURN = LR_REC
                           ).
  ...
  PERFORM F_GET_CONTENT USING LV_VBELN
                     CHANGING LT_HTML_CON.
*  LT_HTML_CON = GET_CONTENT( LV_VBELN ).

  LOOP AT LR_REC INTO DATA(LS_RECEI).
    LS_EMAIL = LS_RECEI-LOW.
    APPEND LS_EMAIL TO LT_REC.
  ENDLOOP.
*
  LOOP AT LR_CC INTO DATA(LS_CC).
    LS_EMAIL = LS_CC-LOW.
  ...
  LCL_UTIL->SEND_MAIL( I_SUBJECT      = LV_SUBJECT
                       ...
                       I_SENDER_EMAIL = 'SDS@DAIKIN.CO.TH'
                       I_SENDER_NAME  = 'SDS'
                       IT_RECEIVER    = LT_REC
                       IT_CC          = LT_CC  ).
...
*&      <-- LT_HTML_CON
*&---------------------------------------------------------------------*
FORM F_GET_CONTENT  USING    LV_VBELN
                    CHANGING LT_HTML_CON TYPE SOLI_TAB.

  DATA : LS_CONTENT TYPE SOLI.

  CLEAR : LS_CONTENT.
  LS_CONTENT-LINE = 'Dear ALL'.
  APPEND LS_CONTENT TO LT_HTML_CON.
  ...
  CLEAR : LS_CONTENT.
  LS_CONTENT-LINE = 'Please find the attachment'.
  APPEND LS_CONTENT TO LT_HTML_CON.
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form F_SEND_EMAIL_MTD
*&---------------------------------------------------------------------*
*& text
*&---------------------------------------------------------------------*
*&      --> US_BIL_INVOICE_HD_GEN_BIL_NUMB
*&      --> LF_BIN_FILESIZE
*&      --> LF_BIN_FILE
*&---------------------------------------------------------------------*
FORM F_SEND_EMAIL_MTD  USING LV_VBELN        TYPE VBRK-VBELN
                             LV_KUNNR        TYPE VBRK-KUNRG
                             LV_BIN_FILESIZE TYPE I
                             LV_BIN_FILE     TYPE XSTRING.
  DATA : LV_FM_NAME TYPE RS38L_FNAM,
         LV_REPEAT  TYPE C LENGTH 1.
  ...
  DATA : LV_HOME_PRO  TYPE KNA1-KUNNR,
         LV_POWER_BUY TYPE KNA1-KUNNR.
  ...
  CONSTANTS : BEGIN OF LC_CON1,
                I_REPID TYPE CHAR20 VALUE 'MTD_EMAIL',
                I_PARAM TYPE CHAR20 VALUE 'SMP',
              END OF LC_CON1.

  CONSTANTS : BEGIN OF LC_CON2,
                I_REPID TYPE CHAR20 VALUE 'MTD_SPECIAL',
                I_PARAM TYPE CHAR20 VALUE 'MTD_HOME',
              END OF LC_CON2.

  CONSTANTS : BEGIN OF LC_CON3,
                I_REPID TYPE CHAR20 VALUE 'MTD_SPECIAL',
                I_PARAM TYPE CHAR20 VALUE 'MTD_EMAIL_HOME',
              END OF LC_CON3.

  CONSTANTS : BEGIN OF LC_CON4,
                I_REPID TYPE CHAR20 VALUE 'MTD_SPECIAL',
                I_PARAM TYPE CHAR20 VALUE 'MTD_POWER',
              END OF LC_CON4.

  CONSTANTS : BEGIN OF LC_CON5,
                I_REPID TYPE CHAR20 VALUE 'MTD_SPECIAL',
                I_PARAM TYPE CHAR20 VALUE 'MTD_EMAIL_POWER',
              END OF LC_CON5.
  ...
  CONCATENATE : 'MTD Billing' LV_DO '/' LV_VBELN INTO LV_SUBJECT SEPARATED BY SPACE.

  LS_ATTACH_FILE-GROUP    = 1.
  CONCATENATE 'INV_' LV_VBELN INTO LS_ATTACH_FILE-FILENAME.
  LS_ATTACH_FILE-LEN      = LV_BIN_FILESIZE.


  LCL_UTIL->GET_CONFIG_001( EXPORTING I_REPID             = LC_CON2-I_REPID
                                      I_SINGLE_VALUE_FLAG = ABAP_TRUE
                                      I_PARAM             = LC_CON2-I_PARAM
                             CHANGING C_RETURN            = LV_HOME_PRO
                           ).

  LCL_UTIL->GET_CONFIG_001( EXPORTING I_REPID             = LC_CON4-I_REPID
                                      I_SINGLE_VALUE_FLAG = ABAP_TRUE
                                      I_PARAM             = LC_CON4-I_PARAM
                             CHANGING C_RETURN            = LV_POWER_BUY
                            ).

  IF     LV_HOME_PRO  EQ LV_KUNNR. "Case Home Pro Email
    LCL_UTIL->GET_CONFIG_001( EXPORTING I_REPID   = LC_CON3-I_REPID
                                        I_PARAM   = LC_CON3-I_PARAM
                               CHANGING CR_RETURN = LR_REC
                             ).
  ELSEIF LV_POWER_BUY EQ LV_KUNNR. "Case Power Buy Email
    LCL_UTIL->GET_CONFIG_001( EXPORTING I_REPID   = LC_CON5-I_REPID
                                        I_PARAM   = LC_CON5-I_PARAM
                               CHANGING CR_RETURN = LR_REC
                             ).
  ELSE. "Case Other
    LCL_UTIL->GET_CONFIG_001( EXPORTING I_REPID   = LC_CON-I_REPID
  ...
  LS_BIL_INVOICE = LCL_DATA=>GET_BILLING_DETAIL( I_VBELN = LV_VBELN
                                                 I_KUNNR = LV_KUNNR ).
*                                                 I_DATA  = LS_PRINT_DATA_TO_REA ).
  ...
  APPEND LS_ATTACH_FILE TO LT_ATTACH_FILE.
  CLEAR : LS_ATTACH_FILE.

  LS_ATTACH_FILE          = LCL_DATA=>CALL_SMART_FORM( I_DATA = LS_BIL_INVOICE ).
  LS_ATTACH_FILE-GROUP    = 2.
  CONCATENATE 'REC_' LV_VBELN INTO LS_ATTACH_FILE-FILENAME.
  APPEND LS_ATTACH_FILE TO LT_ATTACH_FILE.
