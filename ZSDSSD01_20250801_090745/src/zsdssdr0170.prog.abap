*-----------------------------------------------------------------------
*  Program ID         : ZSDSSDR0170
*  Creation Date      : 23.06.2024
*  Author             : Jakarin S.
*  Add-on ID          : ZSDF001
*  Description        : Print DO no Serial by Sales Group and Office
*  Purpose            :
*  Copied from        : ZP_SD_DO_OG_SERIAL
*  Restriction        :
*-----------------------------------------------------------------------
*  CHANGE HISTORY
*-----------------------------------------------------------------------
...
*-----------------------------------------------------------------------
REPORT ZSDSSDR0170 MESSAGE-ID ZSDSSD01.
...
SELECT-OPTIONS : S_VKBUR FOR TVBVK-VKBUR,
                 S_VKGRP FOR TVBVK-VKGRP.
...
FORM F_GET_DATA.
  ...
  SELECT TVKBT~VKBUR
         ...
         TVGRT~BEZEI
    FROM TVBVK
    INNER JOIN TVGRT  ON ( TVBVK~VKGRP  EQ TVGRT~VKGRP AND "get sale group description
                           TVGRT~SPRAS EQ SY-LANGU )
    INNER JOIN TVKBT  ON ( TVBVK~VKBUR  EQ TVKBT~VKBUR AND "get sale office description
                           TVKBT~SPRAS EQ SY-LANGU )
    INTO TABLE GT_RESULT
    WHERE TVBVK~VKBUR IN S_VKBUR
      AND TVBVK~VKGRP IN S_VKGRP.
ENDFORM.                    "f_get_data
*&---------------------------------------------------------------------*
*&      Form  pf_alv_grid
*&---------------------------------------------------------------------*
...
FORM F_SAVE_DATA.
  ...
  DATA : LS_ZSDSSDT008 TYPE ZSDSSDT008,
         LT_ZSDSSDT008 TYPE TABLE OF ZSDSSDT008.
...
*  IF lt_result[] IS NOT INITIAL.
    SELECT *
      FROM ZSDSSDT008
      INTO TABLE LT_ZSDSSDT008
      FOR ALL ENTRIES IN LT_RESULT
      WHERE VKBUR EQ LT_RESULT-VKBUR
        AND VKGRP EQ LT_RESULT-VKGRP.

    LOOP AT LT_RESULT INTO GS_RESULT.
      READ TABLE LT_ZSDSSDT008 INTO LS_ZSDSSDT008
      WITH KEY VKBUR = GS_RESULT-VKBUR
               VKGRP = GS_RESULT-VKGRP.
      IF SY-SUBRC NE 0.
        LS_ZSDSSDT008-ERNAM = SY-UNAME.
        LS_ZSDSSDT008-ERDAT = SY-DATUM.
      ENDIF.

      LS_ZSDSSDT008-VKBUR = GS_RESULT-VKBUR.
      LS_ZSDSSDT008-VKGRP = GS_RESULT-VKGRP.
      LS_ZSDSSDT008-NOSER = GS_RESULT-CHECK.
      LS_ZSDSSDT008-AENAM = SY-UNAME.
      LS_ZSDSSDT008-AEDAT = SY-DATUM.

      MODIFY ZSDSSDT008 FROM LS_ZSDSSDT008.
...
*----------------------------------------------------------------------*
FORM F_GET_ADDITIONAL_DATA .
  DATA : BEGIN OF LS_ZSDSSDT008,
    VKBUR TYPE ZSDSSDT008-VKBUR,
    VKGRP TYPE ZSDSSDT008-VKGRP,
    NOSER TYPE ZSDSSDT008-NOSER,
  END OF LS_ZSDSSDT008.
  DATA LT_ZSDSSDT008 LIKE SORTED TABLE OF LS_ZSDSSDT008 WITH UNIQUE KEY VKBUR VKGRP.
  ...
  SELECT VKBUR
    ...
    WHERE VKBUR EQ GT_RESULT-VKBUR
      AND VKGRP EQ GT_RESULT-VKGRP.

  LOOP AT GT_RESULT INTO GS_RESULT.
    LV_TABIX = SY-TABIX.

    READ TABLE LT_ZSDSSDT008 INTO LS_ZSDSSDT008
    WITH TABLE KEY VKBUR = GS_RESULT-VKBUR
                   VKGRP = GS_RESULT-VKGRP.
    IF SY-SUBRC EQ 0.
      GS_RESULT-CHECK = LS_ZSDSSDT008-NOSER.
      MODIFY GT_RESULT FROM GS_RESULT INDEX LV_TABIX
                               TRANSPORTING CHECK.
    ENDIF.

    CLEAR : GS_RESULT,LS_ZSDSSDT008.
  ENDLOOP.
