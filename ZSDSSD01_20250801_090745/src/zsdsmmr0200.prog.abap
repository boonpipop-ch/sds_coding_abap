...
*  Add-on ID          : ZSDI007
*  Description        : Report and Export interface Delivery and Return Delivery FG to SMP
*  Purpose            :
...
*& C O N S T A N T
*&-----------------------------------------------------------------------------------*
CONSTANTS : GC_REPID       TYPE REPID         VALUE 'ZSDSMMR0200',
...
FORM F_MAP_DATA.
  ...
  LOOP AT GT_LIKP INTO GS_LIKP WHERE LGORT NE ''.
    ...
    IF GS_LIKP-SERAIL EQ 'ZSDS'.
      ...
      LOOP AT GT_SER01_OBJK INTO GS_SER01_OBJK WHERE LIEF_NR EQ GS_ITAB-VBELN
                                               AND POSNR EQ GS_ITAB-POSNR.


        GS_ITAB-SERNR = GS_SER01_OBJK-SERNR.
        GS_ITAB-LFIMG = 1 .
    ...
    IF GS_LIKP-SERAIL EQ 'ZSDS'.
      ...
      LOOP AT GT_SER01_OBJK INTO GS_SER01_OBJK1 WHERE LIEF_NR EQ GS_ITAB-VBELN
                                               AND POSNR EQ GS_ITAB-POSNR.
...
*                            gs_text_export+l_pos(1) = g_space.
    ...
    CONCATENATE 'SDSRT_' SY-DATUM '_' SY-TIMLO '.txt' INTO GS_PATH_NAME.
...
*  ZCL_SDSCA_UTIL_SDS=>GET_CONFIG_001( EXPORTING   I_REPID             = 'ZSDSMMR0200' "LC_CON-REPID
...
*  IF sy-sysid = 'F36'.
*     LV_PATH_FILE = '/interface/Z_DS_Dev/SDS_Dev/20_OUTBOUND/WMS_SMP/ZSDI007'.
*  ELSEIF sy-sysid = 'F46'.
*     LV_PATH_FILE = '/interface/Z_DS/SDS/20_OUTBOUND/WMS_SMP/ZSDI007'.
*  ELSE.
...
*        APPEND LS_FILE TO LT_FILE.
  LV_PATH_FILE = P_PATH.
  CONCATENATE 'SDSTRT_' SY-DATUM '_' SY-TIMLO '.txt' INTO LV_PATH.
