...
CLASS LCL_DATA IMPLEMENTATION.
  ...
  METHOD START_PROCESS.
    ...
    DATA : S_PRINF TYPE RANGE OF ZSDSSDT026-PRINF.

    IF R1 EQ ABAP_TRUE.
      S_PRINF =  VALUE #( ( SIGN  = 'I' OPTION = 'NE' LOW = ABAP_TRUE ) ).
    ELSE.
      CLEAR : S_PRINF.
    ENDIF.
    ...
    SELECT A~VBELN,
           ...
           VBRK~NETWR,
           A~PRINF
     FROM ZSDSSDT026 AS A
*      INNER JOIN VBRP  ON A~VBELN EQ VBRP~VBELN
      INNER JOIN VBRK  ON A~VBELN     EQ VBRK~VBELN
      INNER JOIN KNA1  ON VBRK~KUNRG  EQ KNA1~KUNNR
      WHERE A~VBELN    IN @S_VBELN
        AND A~REFDC    IN @S_REFDC
        AND A~PRINF    IN @S_PRINF
...
*
*    SELECT TVKBT~BEZEI AS SLEGP ,
*           TVGRT~BEZEI AS SLEOF ,
*           A~VKGRP,
*           A~VKBUR
...
*      IF SY-SUBRC = 0.
*        <LFS_DATA>-SLEGP = LS_SALESGP_SALESOF1-SLEGP.
*        <LFS_DATA>-SLEOF = LS_SALESGP_SALESOF1-SLEOF.
*      ENDIF.
*    ENDLOOP.
