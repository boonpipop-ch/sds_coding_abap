*-----------------------------------------------------------------------
*  Program ID         : ZSDSSDR0120
*  Creation Date      : 21.05.2024
*  Author             : Kittirat C.
*  Add-on ID          : ZSDF004
*  Description        : Program Print Debit Note
*  Purpose            :
*  Copied from        :
*  Restriction        :
*-----------------------------------------------------------------------
...
*-----------------------------------------------------------------------
* 13.01.2025  F36K911044  Kittirat C.  CH01 :Change calculation of
*                                      increase price
*-----------------------------------------------------------------------
REPORT ZSDSSDR0120.
...
CONSTANTS:
  ...
  GC_INTFNO               TYPE ZSDSCAC004-INTFNO  VALUE 'SDF004',
...
* RANGE
*----------------------------------------------------------------------*
DATA: GRT_DOCTYPE                   TYPE RANGE OF VBRK-FKART  ##NEEDED.
...
FORM F_PRINT USING UF_RETCODE   TYPE SY-SUBRC
  ...
  DATA: LS_PRINT_DATA_TO_READ TYPE LBBIL_PRINT_DATA_TO_READ,
        LS_DEBIT_NOTE         TYPE LBBIL_INVOICE,
...
*       call smartform debit note for generating PDF
        CALL FUNCTION LF_FM_NAME
          EXPORTING
            ARCHIVE_INDEX      = TOA_DARA
            ARCHIVE_PARAMETERS = ARC_PARAMS
...
*         error handling
          ...
          PERFORM F_PROTOCOL_UPDATE.
        ELSE.
          PERFORM F_GENERATE_PDF USING LS_DEBIT_NOTE
                                       LS_JOB_INFO
                                       LF_SUBRC.
        ENDIF.
      ENDIF.
...
FORM F_GET_INVOICE_DATA
              USING US_PRINT_DATA_TO_READ TYPE LBBIL_PRINT_DATA_TO_READ
                    US_DEBIT_NOTE         TYPE LBBIL_INVOICE
                    UF_RETCODE            TYPE SY-SUBRC.
* read print data
...
FORM F_GET_CONSTANTS.
  ...
  LOOP AT GT_CONST ASSIGNING FIELD-SYMBOL(<L_CONST>).
    CASE <L_CONST>-PARAM.
      ...
      WHEN 'DN_DOCTYPE'.
        APPEND INITIAL LINE TO GRT_DOCTYPE
...
*& Prepare Form Data
*&---------------------------------------------------------------------*
FORM F_PREPARE_FORM_DATA USING US_DEBIT_NOTE  TYPE LBBIL_INVOICE  ##CALLED
                               US_HEADER      TYPE ZSDSSDS031
                               US_FOOTER      TYPE ZSDSSDS032
                               UT_ITEM        TYPE ZSDSSDS033_TT.
  ...
  CHECK NOT US_DEBIT_NOTE IS INITIAL.

  PERFORM F_GET_CONSTANTS.

  IF NOT US_DEBIT_NOTE-IT_GEN IS INITIAL.
    SELECT FROM VBRP
    FIELDS VBELN,
           POSNR,
           VKGRP,
           AUFNR,
           AUGRU_AUFT
    FOR ALL ENTRIES IN @US_DEBIT_NOTE-IT_GEN
    WHERE VBELN EQ @US_DEBIT_NOTE-IT_GEN-BIL_NUMBER
      AND POSNR EQ @US_DEBIT_NOTE-IT_GEN-ITM_NUMBER
    INTO TABLE @LT_VBRP.
    IF SY-SUBRC EQ 0.
...
*----------------------------------------------------------------------*
  ...
  FIELDS XBLNR
  WHERE VBELN EQ @US_DEBIT_NOTE-HD_GEN-BIL_NUMBER
  INTO @US_HEADER-DEBIT_NOTE_NO.

  US_HEADER-DOC_TYPE      = US_DEBIT_NOTE-HD_GEN-BIL_TYPE.
  US_HEADER-CUSTOMER_NO   = US_DEBIT_NOTE-HD_GEN-PAYER.
  IF NAST-SPRAS EQ '2'.
    US_HEADER-COMPANY_TAXID  = |เลขประจำตัวผู้เสียภาษี { US_DEBIT_NOTE-HD_GEN-T001_VAT+2 }|.
  ELSE.
    US_HEADER-COMPANY_TAXID  = |TAX ID NO. { US_DEBIT_NOTE-HD_GEN-T001_VAT+2 }|.
  ENDIF.

  PERFORM F_WRITE_DATE  USING US_DEBIT_NOTE-HD_GEN-BIL_DATE
                              US_HEADER-DEBIT_NOTE_DATE.

  PERFORM F_READ_INVOICE_NO_DATE USING US_DEBIT_NOTE
  ...
  "get tax rate
  READ TABLE US_DEBIT_NOTE-HD_KOND INTO DATA(LS_KOND)
  WITH KEY KAPPL  = 'V'
           KSCHL  = 'MWST'.
  IF SY-SUBRC EQ 0.
  ...
  PERFORM F_GET_COMPANY_INFO  USING US_DEBIT_NOTE
                                    US_HEADER.

  PERFORM F_GET_CUSTOMER_INFO USING US_DEBIT_NOTE
                                    US_HEADER.

*----------------------------------------------------------------------*
*  Prepare Items
*----------------------------------------------------------------------*
  PERFORM F_PREPARE_FORM_ITEMS USING US_DEBIT_NOTE
                                     US_FOOTER
                                     UT_ITEM.
...
* Implement suitable error handling here
  ...
  LF_XBLNR  = |{ US_HEADER-DEBIT_NOTE_NO ALPHA = OUT }|.  CONDENSE LF_XBLNR.
  LF_PAYER  = |{ US_DEBIT_NOTE-HD_GEN-PAYER ALPHA = OUT }|.  CONDENSE LF_PAYER.
  ...
  IF NOT US_DEBIT_NOTE-HD_REF-ORDER_NUMB IS INITIAL.
    SELECT SINGLE FROM VBAK
    FIELDS BNAME
    WHERE VBELN EQ @US_DEBIT_NOTE-HD_REF-ORDER_NUMB
    INTO @LF_BNAME.
    IF SY-SUBRC EQ 0.
...
*& Get Company Info
*&---------------------------------------------------------------------*
FORM F_GET_COMPANY_INFO  USING  US_DEBIT_NOTE TYPE LBBIL_INVOICE
                                US_HEADER  TYPE ZSDSSDS031.
  ...
  FIELDS ADRNR
  WHERE BUKRS EQ @US_DEBIT_NOTE-HD_ORG-COMP_CODE
  INTO @LF_ADRNR.
  ...
  IF SY-SUBRC EQ 0.
    ...
    IF LF_BUPLA EQ '0000'.  "Head Office
      ...
      IF SY-SUBRC EQ 0.
        ...
        WITH KEY BUKRS      = US_DEBIT_NOTE-HD_ORG-COMP_CODE
                 BRANCH     = LF_BUPLA
                 BUPLA_TYPE = SPACE
...
*& Get Customer Info
*&---------------------------------------------------------------------*
FORM F_GET_CUSTOMER_INFO  USING  US_DEBIT_NOTE  TYPE LBBIL_INVOICE
                                 US_HEADER      TYPE ZSDSSDS031.
  DATA: LT_ADRC             TYPE TT_ADRC.
  DATA: LS_ADRC             LIKE LINE OF LT_ADRC.
  ...
  READ TABLE US_DEBIT_NOTE-HD_ADR INTO LS_HD_ADR        "#EC CI_SORTSEQ
  ...
  IF NOT US_DEBIT_NOTE-HD_GEN-PAYER IN GRT_ONETIME_GET_TAXID.
    SELECT SINGLE FROM KNA1
    FIELDS KUNNR,
           STCD3
    WHERE KUNNR EQ @US_DEBIT_NOTE-HD_GEN-PAYER
    INTO @LS_KNA1.
  ELSE.
  ...
  ELSE.
    ...
    FIELDS  FITHA_PBUPL_D~KUNNR,
            ...
            FITHA_PBUPL_D~DEFAULT_BRANCH,
            FITHA_PBUPL_D_T~DESCRIPTION
    WHERE FITHA_PBUPL_D~KUNNR EQ @US_DEBIT_NOTE-HD_GEN-PAYER
  ...
  "Get customer branch
  IF NOT US_DEBIT_NOTE-HD_GEN-PAYER IN GRT_CUST_NOT_SHOW_BRANCH AND
...
*& Prepare Form Items
*&---------------------------------------------------------------------*
FORM F_PREPARE_FORM_ITEMS  USING  US_DEBIT_NOTE TYPE LBBIL_INVOICE
                                  US_FOOTER     TYPE ZSDSSDS032
                                  UT_ITEM       TYPE ZSDSSDS033_TT.
  ...
  FIELDS VBELN,
         ...
         MWSBP
  WHERE VBELN EQ @US_DEBIT_NOTE-HD_GEN-BIL_NUMBER
  INTO TABLE @LT_VBRP_DN.
  IF SY-SUBRC EQ 0.
    SORT LT_VBRP_DN BY VBELN POSNR ASCENDING.
  ENDIF.

  IF NOT US_DEBIT_NOTE-HD_REF-VBELN_VG2 IS INITIAL.
    LF_VBELN_INV  = US_DEBIT_NOTE-HD_REF-VBELN_VG2.

  ELSEIF NOT US_DEBIT_NOTE-HD_REF-ORDER_NUMB IS INITIAL.
    "get invoice no.
    SELECT FROM VBFA
    FIELDS VBELV,
           POSNV,
  ...
  IF NOT GRT_DOCTYPE IS INITIAL AND
     US_DEBIT_NOTE-HD_GEN-BIL_TYPE IN GRT_DOCTYPE.
    ...
    ENDIF.
  ELSE.
    "doctype not in genc use items from DN
    LT_VBRP = LT_VBRP_DN.
  ENDIF.

  PERFORM F_READ_MERGE_ITEMS  USING US_DEBIT_NOTE
                                    LF_FOUND_ZH16.
  ...
  IF LF_FOUND_ZH16  EQ ABAP_TRUE. "text ZH16 found
    ...
    LOOP AT LT_VBRP ASSIGNING <L_VBRP>.
      ...
      IF SY-SUBRC NE 0.
        "New material
        LF_INDEX              = LF_INDEX + 1.
        LS_ITEM-DEBIT_NOTE_NO = <L_VBRP>-VBELN.
        ...
        PERFORM F_READ_MAT_DESC USING US_DEBIT_NOTE
                                    <L_VBRP>
                                    LS_ITEM-MODEL_DESC.

        PERFORM F_READ_QUANTITY USING US_DEBIT_NOTE
                                      <L_VBRP>
                                      LS_ITEM-QUANTITY.

        PERFORM F_READ_UNIT USING US_DEBIT_NOTE
                                  <L_VBRP>
                                  LS_ITEM-SALES_UNIT
  ...
  ELSE. "Text ZH16 not found
    ...
    LOOP AT LT_VBRP ASSIGNING <L_VBRP> WHERE UEPOS IS INITIAL.
      ...
      LOOP AT LT_VBRP ASSIGNING <L_BOM> WHERE UEPOS EQ <L_VBRP>-POSNR.  "#EC CI_NESTED
        ...
        LS_BOM-DEBIT_NOTE_NO  = <L_BOM>-VBELN.
      ...
      IF NOT LS_BOM IS INITIAL. "has BOM
        ...
        WITH KEY  DEBIT_NOTE_NO = LS_BOM-DEBIT_NOTE_NO
      ...
      ELSE.
        ...
        READ TABLE UT_ITEM ASSIGNING <L_ITEM>
        WITH KEY  DEBIT_NOTE_NO = <L_VBRP>-VBELN
  ...
  PERFORM F_READ_INVOICE_AMOUNT USING US_DEBIT_NOTE
                                      LT_VBRP_INV
                                      US_FOOTER-INVOICE_AMOUNT.

  "calculate DN amount
  LOOP AT LT_VBRP_DN ASSIGNING <L_VBRP>.
*BOD CH01
*    IF US_DEBIT_NOTE-HD_GEN-BIL_TYPE IN GRT_DOCTYPE.
*      US_FOOTER-PRICE_INCREASE  = US_FOOTER-PRICE_INCREASE + <L_VBRP>-KZWI5.
*    ELSE.
*      US_FOOTER-PRICE_INCREASE  = US_FOOTER-PRICE_INCREASE + <L_VBRP>-KZWI4.
*    ENDIF.
*EOD CH01
    US_FOOTER-PRICE_INCREASE  = US_FOOTER-PRICE_INCREASE + <L_VBRP>-NETWR.  "CH01 ADD
    US_FOOTER-VALUE_ADD_TAX   = US_FOOTER-VALUE_ADD_TAX + <L_VBRP>-MWSBP.
  ENDLOOP.

  US_FOOTER-TOTAL_AMOUNT  = US_FOOTER-INVOICE_AMOUNT + US_FOOTER-PRICE_INCREASE.
  US_FOOTER-NET_AMOUNT    = US_FOOTER-PRICE_INCREASE + US_FOOTER-VALUE_ADD_TAX.
ENDFORM.
*&---------------------------------------------------------------------*
*& Form F_GENERATE_PDF
*&---------------------------------------------------------------------*
*& Generate PDF file
*&---------------------------------------------------------------------*
FORM F_GENERATE_PDF  USING  US_DEBIT_NOTE TYPE LBBIL_INVOICE
  ...
  IF NOT LF_BIN_FILE IS INITIAL.
    ...
    FIELDS DOCTY
    WHERE FKART EQ @US_DEBIT_NOTE-HD_GEN-BIL_TYPE
    INTO @LF_DOCTY.

    LF_FILENAME = |B{ LF_DOCTY }_{ LF_LANG }_{ SY-DATUM }_{ SY-UZEIT }|.
    LF_FILENAME = |{ LF_FILENAME }_{ US_DEBIT_NOTE-HD_ORG-COMP_CODE }|.
...
*&---------------------------------------------------------------------*
FORM F_READ_MAT_DESC  USING US_DEBIT_NOTE TYPE LBBIL_INVOICE
                            US_VBRP       TYPE TS_VBRP
                            UF_MODEL_DESC TYPE ZSDSSDS033-MODEL_DESC.
  DATA: LT_LINE     TYPE STANDARD TABLE OF TLINE.
  ...
  LF_TDID     = 'ZI06'.
  LF_TDNAME   = |{ US_DEBIT_NOTE-HD_GEN-BIL_NUMBER }{ US_VBRP-POSNR }|.
  LF_TDOBJECT = 'VBBP'.
...
*& Read Quantity
*&---------------------------------------------------------------------*
FORM F_READ_QUANTITY  USING  US_DEBIT_NOTE  TYPE LBBIL_INVOICE
                             US_VBRP        TYPE TS_VBRP
                             UF_QUANTITY    TYPE ZSDSSDS033-QUANTITY.
  DATA: LT_LINE     TYPE STANDARD TABLE OF TLINE.
  DATA: LS_LINE     LIKE LINE OF LT_LINE.
  DATA: LF_TDID     TYPE THEAD-TDID.
  DATA: LF_TDNAME   TYPE THEAD-TDNAME.
...
*& text
*&---------------------------------------------------------------------*
FORM F_READ_UNIT  USING US_DEBIT_NOTE       TYPE LBBIL_INVOICE
                        US_VBRP             TYPE TS_VBRP
                        UF_SALES_UNIT       TYPE ZSDSSDS033-SALES_UNIT
                        UF_SALES_UNIT_TEXT  TYPE ZSDSSDS033-SALES_UNIT_TEXT.
  DATA: LT_LINE     TYPE STANDARD TABLE OF TLINE.
  DATA: LS_LINE     LIKE LINE OF LT_LINE.
  DATA: LF_TDID     TYPE THEAD-TDID.
  DATA: LF_TDNAME   TYPE THEAD-TDNAME.
...
*& Read Merge Items
*&---------------------------------------------------------------------*
FORM F_READ_MERGE_ITEMS  USING US_DEBIT_NOTE  TYPE LBBIL_INVOICE
  ...
  LF_TDID     = 'ZH16'.
  LF_TDNAME   = US_DEBIT_NOTE-HD_GEN-BIL_NUMBER.
  LF_TDOBJECT = 'VBBK'.
...
*& text
*&---------------------------------------------------------------------*
FORM F_READ_INVOICE_AMOUNT  USING  US_DEBIT_NOTE      TYPE LBBIL_INVOICE
                                   UT_VBRP            TYPE TT_VBRP
                                   UF_INVOICE_AMOUNT  TYPE ZSDSSDS032-INVOICE_AMOUNT.
  DATA: LT_LINE     TYPE STANDARD TABLE OF TLINE.
  DATA: LS_LINE     LIKE LINE OF LT_LINE.
  DATA: LF_TDID     TYPE THEAD-TDID.
  DATA: LF_TDNAME   TYPE THEAD-TDNAME.
...
*&---------------------------------------------------------------------*
FORM F_READ_INVOICE_NO_DATE  USING  US_DEBIT_NOTE   TYPE LBBIL_INVOICE
                                    UF_INVOICE_NO   TYPE ZSDSSDS031-INVOICE_NO
                                    UF_INVOICE_DATE TYPE ZSDSSDS031-INVOICE_DATE.
  TYPES: BEGIN OF LTS_VBFA,
           VBELV TYPE VBFA-VBELV,
           POSNV TYPE VBFA-POSNV,
           VBELN TYPE VBFA-VBELN,
  ...
  IF UF_INVOICE_NO IS INITIAL OR
    ...
    IF NOT US_DEBIT_NOTE-HD_REF-VBELN_VG2 IS INITIAL.
      ...
      FIELDS FKDAT,
             XBLNR
      WHERE VBELN EQ @US_DEBIT_NOTE-HD_REF-VBELN_VG2
      INTO @LS_VBRK.
      IF SY-SUBRC EQ 0.
