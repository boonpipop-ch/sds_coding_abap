...
FORM F_CALL_TRANSECTION.
  ...
  IF SY-SUBRC = 0.
    PERFORM BDC_DYNPRO   USING  'SAPMV60A'   '0401'.
    PERFORM BDC_FIELD    USING  'BDC_CURSOR' 'VBRK-VBELN'.
    PERFORM BDC_FIELD    USING  'BDC_OKCODE' '=CONTI'.
    PERFORM BDC_FIELD    USING  'VBRK-VBELN' GS_RESULT-VBELN.

    PERFORM BDC_TRANSACTION USING 'VFR3'.
  ENDIF.
...
*&---------------------------------------------------------------------*
FORM F_SAVE .
  LOOP AT GT_RESULT ASSIGNING FIELD-SYMBOL(<LFS_DATA>) WHERE CHECK  EQ ABAP_TRUE AND
                                                             STATUS EQ GC_WARN.
