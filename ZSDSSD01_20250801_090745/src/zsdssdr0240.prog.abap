*-----------------------------------------------------------------------
*  Program ID         : ZSDSSDR0240
*  Creation Date      : 30.07.2024
*  Author             : Zulkiff B.(Eviden)
*  Add-on ID          :
*  Description        : This is a program to upload Quotation DOC from ECC6
*  Purpose            : To upload Quotation DOC from ECC6
*  Copied from        : N/A
*  Restriction        : N/A
*-----------------------------------------------------------------------
*  CHANGE HISTORY
...
CONSTANTS:
  ...
  GC_TCODE TYPE  SY-TCODE  VALUE 'ZSDSSD016',
*  GC_ZQ01  TYPE  AUART     VALUE 'ZQ01',
...
*----------------------------------------------------------------------*
CONSTANTS:
  GC_VA23 TYPE  SY-TCODE    VALUE 'VA23',
...
*---Fill data from Template Header to Header Data--------
        ...
        WHEN 'DOC_TYPE'.
*          LS_DATA-HEAD-DOC_TYPE = <L_FIELD>."GC_ZQ01.
...
*    LS_SCHED TYPE  TS_SCHED,
  ...
  FIELD-SYMBOLS:
    <L_DATA>  TYPE  TS_DATA,
    <L_ITEM>  TYPE  TS_ITEM  ##NEEDED,
    <L_SCHED> TYPE  TS_SCHED  ##NEEDED.
...
*----------------------------------------------------------------------*
FORM F_ALV_SORT_RESULT  CHANGING CT_SORT TYPE LVC_T_SORT ##NEEDED.
...
*    LV_BILLPLAN_HD_FLAG  TYPE CHAR1,
    LT_MIGRATION_CONDTYP TYPE KSCHL_RAN_TAB.

  TYPES:
    BEGIN OF LTY_SERVMAT_ITEMNO   ##NEEDED,
      DIGIT1     TYPE NUMC1,
...
*-------------------Header-----------------------------
  LS_HEADER = VALUE #( NAME        = US_DATA-HEAD-REF_1"US_DATA-HEAD-BNAME
                       DOC_DATE    = SY-DATUM
                       QT_VALID_F  = US_DATA-HEAD-CT_VALID_F
                       QT_VALID_T  = US_DATA-HEAD-CT_VALID_T
                       ...
                       REF_1       = US_DATA-HEAD-QUOTATION "US_DATA-HEAD-REF_1
...
**ZDH5
  ...
  LOOP AT LT_DATA_ITEM INTO LS_DATA_ITEM .
...
* Call VB23
  PERFORM F_AUTHORIZE_CHECK USING GC_VA23.
  CALL TRANSACTION GC_VA23 WITHOUT AUTHORITY-CHECK
