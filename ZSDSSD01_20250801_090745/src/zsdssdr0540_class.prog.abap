...
*              VBAP~NETPR,
      ...
      FROM VBAK
      LEFT JOIN  FPLA ON VBAK~VBELN EQ FPLA~VBELN AND
                         FPLA~FPART EQ 'Z2' AND
                         FPLA~RFPLN EQ @SPACE
      LEFT JOIN  FPLT ON FPLA~FPLNR EQ FPLT~FPLNR
      LEFT JOIN  ZSDSSDT022 AS A ON FPLA~VBELN EQ A~VBELN
                                AND ( FPLT~FPLTR EQ A~FPLTR or
                                     A~FPLTR     EQ '000000' )
              ...
              AND NOT EXISTS ( SELECT VBELN
                                ...
                                WHERE A~VBELN EQ VBAK~VBELN
                                  AND ( A~FPLTR EQ FPLT~FPLTR OR
                                        A~FPLTR EQ '000000' )
                                  AND ( A~STATU EQ 'SAV' OR
                                        A~STATU EQ 'APP' ) )
...
*
      ...
      LEFT JOIN  FPLA ON VBAK~VBELN EQ FPLA~VBELN AND
                     ...
                     AND FPLA~RFPLN EQ @SPACE
      LEFT JOIN  FPLT ON FPLA~FPLNR EQ FPLT~FPLNR
      LEFT JOIN  ZSDSSDT022 AS B ON VBAK~VBELN EQ B~VBELN
                                AND FPLT~FPLTR EQ B~FPLTR
                    WHERE  VBAK~VBELN IN @S_VBELN
                    AND    VBAK~ERDAT IN @S_ERDAT
                    AND    VBAK~ERNAM IN @S_ERNAM
                    AND    VBAK~AUART IN @S_AUART
  ...
  ENDIF.
    ...
    LOOP AT GT_RESULT ASSIGNING FIELD-SYMBOL(<LFS_DATA>).
      ...
      IF <LFS_DATA>-STATU_S IS NOT INITIAL.
        <LFS_DATA>-STATU = <LFS_DATA>-STATU_S.
      ENDIF.
    ENDLOOP.
...
*  CALL METHOD document->new_line.
  ...
  METHOD SAVE.
    ...
    LOOP AT GT_RESULT INTO DATA(LS_RESULT) WHERE CHECK EQ ABAP_TRUE.
      ...
      ELSE.
        LS_ZSDSSDT022-ERNAM = SY-UNAME.
        LS_ZSDSSDT022-ERDAT = SY-DATUM.
        LS_ZSDSSDT022-ERZET = SY-UZEIT.
      ENDIF.

      LS_ZSDSSDT022-AENAM = SY-UNAME.
      LS_ZSDSSDT022-AEDAT = SY-DATUM.
      LS_ZSDSSDT022-AEZET = SY-UZEIT.
