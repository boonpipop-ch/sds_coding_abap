...
CLASS LCL_DATA IMPLEMENTATION.
  ...
  METHOD START_PROCESS.
    ...
    SELECT VBAP~VBELN AS SONUM,
      ...
      INNER JOIN FPLA ON VBAP~VBELN EQ FPLA~VBELN
                     AND FPLA~FPART EQ 'Z2'
      LEFT JOIN LIPS  ON VBAP~VBELN EQ LIPS~VGBEL
                     AND VBAP~POSNR EQ LIPS~VGPOS
      LEFT JOIN VBRP  ON VBAP~VBELN EQ VBRP~AUBEL
                     AND VBAP~POSNR EQ VBRP~AUPOS
      LEFT JOIN VBRK  ON VBRP~VBELN EQ VBRK~VBELN
      WHERE VBAK~VBTYP EQ @GC_C
    ...
    SELECT TVKBT~BEZEI AS DESOF ,
           TVGRT~BEZEI AS DESGP ,
           A~VKGRP,
           A~VKBUR
...
*    DATA : BEGIN OF LS_BILLING_TYPE,
*
*             BILNO TYPE VBRP-VBELN,
*
*           END OF LS_BILLING_TYPE.
*    DATA : LT_BILLING_TYPE LIKE HASHED TABLE OF LS_BILLING_TYPE WITH UNIQUE KEY BILNO.
*
*    LT_BILLING_TYPE =  CORRESPONDING #( GT_RESULT  DISCARDING DUPLICATES ).
*
*    SELECT A~BILNO,
*           VBRK~FKART
*
*     FROM @LT_BILLING_TYPE AS A
*      INNER JOIN VBRK ON A~BILNO EQ VBRK~VBELN
*      INTO TABLE  @DATA(LT_BILLING_TYPE1).
    ...
    LOOP AT GT_RESULT ASSIGNING FIELD-SYMBOL(<LFS_DATA>).
      ...
      IF SY-SUBRC = 0.
        <LFS_DATA>-DESGP = LS_SALESGP_SALESOF1-DESGP.
        <LFS_DATA>-DESOF = LS_SALESGP_SALESOF1-DESOF.
      ENDIF.

*      READ TABLE LT_BILLING_TYPE1 INTO DATA(LS_BILLING_TYPE1)
*      WITH KEY BILNO = <LFS_DATA>-BILNO.
*      IF SY-SUBRC = 0.
*        <LFS_DATA>-FKART = LS_BILLING_TYPE1-FKART.
*
*        IF <LFS_DATA>-FKART = 'ZS01'.
*          <LFS_DATA>-NETWR = <LFS_DATA>-NETWR * -1.
*        ENDIF.
*
*      ENDIF.

      IF <LFS_DATA>-VBTYP NE 'M'.
        <LFS_DATA>-NETWR = <LFS_DATA>-NETWR * -1.
      ENDIF.
    ...
    SORT GT_RESULT BY SONUM POSNR.
    DELETE ADJACENT DUPLICATES FROM GT_RESULT COMPARING SONUM POSNR.

    SORT GT_RESULT BY SONUM
                      BILNO.
