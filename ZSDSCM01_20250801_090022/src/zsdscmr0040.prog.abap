*-----------------------------------------------------------------------
*  Program ID         : ZSDSCMR0040
*  Creation Date      : 19.12.2024
*  Author             : Wantanee Prateep
*  Add-on ID          :
*  Description        : Out-Bound Warranty to Salesforce for case month
*  Purpose            : Get data Warranty and gen Interface file to
*                       salesforce
*  Copied from        : ZSDSCMR0020
*  Restriction        : N/A
*-----------------------------------------------------------------------
*  CHANGE HISTORY
*-----------------------------------------------------------------------
...
TYPES:
  TT_ZSDSCMC009 TYPE STANDARD TABLE OF ZSDSCMC009,
...
CONSTANTS:
  ...
  GC_TCODE       TYPE  SY-TCODE  VALUE 'ZSDSCM008',
...
SELECTION-SCREEN BEGIN OF BLOCK B1 WITH FRAME TITLE TEXT-S01.
  SELECT-OPTIONS:
    ...
    S_GISTD  FOR ZSDSCMT003-GI_WRT_BEG.
...
FORM F_GET_DATA CHANGING CT_ZSDSCMT003 TYPE TT_ZSDSCMT003
                         CT_ZSDSCMC009 TYPE TT_ZSDSCMC009
...
* ZSDSCMT003 SDS: Warranty Master
  SELECT
   ...
   WHERE ZSDSCMT003~EQUNR IN @S_EQUNR
     AND ZSDSCMT003~UPDFLG = @P_UPDT
     AND ZSDSCMT003~GI_WRT_BEG IN  @S_GISTD
...
* ZSDSCMC009 SDS: Warranty Packages
    SELECT *
      FROM ZSDSCMC009
       FOR ALL ENTRIES IN @CT_ZSDSCMT003
     WHERE WRTPK = @CT_ZSDSCMT003-WRTPK
      INTO TABLE @CT_ZSDSCMC009 .
    IF SY-SUBRC = 0 .
      SORT CT_ZSDSCMC009 BY WRTPK MATKL .
    ENDIF.
...
FORM F_PROCESS_DATA .
  DATA:
    LT_ZSDSCMC009 TYPE TT_ZSDSCMC009,
...
FORM F_SET_DATA  USING UT_ZSDSCMT003 TYPE TT_ZSDSCMT003
                       UT_ZSDSCMC009 TYPE TT_ZSDSCMC009
  ...
  LOOP AT UT_ZSDSCMT003 ASSIGNING FIELD-SYMBOL(<F_ZSDSCMT003>).
    ...
    LS_RESULT-STD_WRT_END = <F_ZSDSCMT003>-GI_WRT_END .
...
*- Set End date for Standard Warranty
    PERFORM F_SET_STD_ENDDATE USING UT_ZSDSCMC009
                                    <F_ZSDSCMT003>
                           CHANGING LS_RESULT .
...
*- Set End date for Extended Warranty
    PERFORM F_SET_EXT_ENDDATE USING UT_ZSDSCMC009
                                    UT_ZSDSCMT006
                                    <F_ZSDSCMT003>
                           CHANGING LS_RESULT .
...
*&---------------------------------------------------------------------*
*& Set Standard End Date by get Warranty Year from table ZSDSCMC009
*&---------------------------------------------------------------------*
FORM F_SET_STD_ENDDATE  USING US_ZSDSCMC009 TYPE TT_ZSDSCMC009
  ...
  LOOP AT GT_MAPFIELD ASSIGNING FIELD-SYMBOL(<LF_MAPFIELD>) .
    ...
    READ TABLE US_ZSDSCMC009 ASSIGNING FIELD-SYMBOL(<LF_CMC002>)
    ...
    IF <LF_CMC002> IS ASSIGNED AND <LF_CMC002>-STWRT > 0 .
      ...
      IF <LF_VALUE> IS ASSIGNED.
        ...
        IF <LF_VALUE_STD_BEG> IS ASSIGNED  .
            IF US_ZSDSCMT003-CUS_REG_BEG IS NOT INITIAL.
...
*&---------------------------------------------------------------------*
FORM F_SET_EXT_ENDDATE  USING   UT_ZSDSCMC009  TYPE TT_ZSDSCMC009
                                UT_ZSDSCMT006  TYPE TT_ZSDSCMT006
                                US_ZSDSCMT003  TYPE TS_ZSDSCMT003
                      CHANGING  CS_RESULT      TYPE TS_RESULT ##NEEDED.
  ...
  DATA: LF_EXT_BEG        TYPE STRING,
        ...
        LF_STD_WRTTY_YEAR TYPE ZSDSCMC009-STWRT,
        LF_EXT_WRTTY_YEAR TYPE ZSDSCMC009-STWRT,
        LF_WRTTY_YEAR     TYPE ZSDSCMC009-STWRT,
  ...
  LOOP AT GT_MAPFIELD ASSIGNING FIELD-SYMBOL(<LF_MAPFIELD>) .
    ...
    IF <LF_CMT006> IS ASSIGNED .
      ...
      IF <LF_VALUE_EXT_END> IS ASSIGNED.
        ...
        "Get Standard warrantly(YEARS) and Matgroup of EquipmentID FROM ZSDSCMC009
        READ TABLE UT_ZSDSCMC009 ASSIGNING FIELD-SYMBOL(<LF_CMC002>)
          WITH KEY WRTPK = US_ZSDSCMT003-WRTPK
                   MATKL = <LF_MAPFIELD>-MATKL
           BINARY SEARCH .
