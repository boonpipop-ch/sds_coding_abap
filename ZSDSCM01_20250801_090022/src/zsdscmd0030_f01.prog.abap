...
FORM F_DISPLAY_INVOICE_CREATE.
  ...
  DATA: LT_INV TYPE STANDARD TABLE OF LTYP_INV,
        LS_INV TYPE LTYP_INV.
...
*   WHERE POSID = @P_POSID.
  ...
  ELSE.
    MESSAGE TEXT-E12 TYPE 'S' DISPLAY LIKE 'E'. "Project WBS was not found
    RETURN.
  ENDIF.
...
*Get D/O
  ...
  IF SY-SUBRC = 0.
    ...
    SELECT VBRP~VBELN,
           ...
           PA0001~ENAME
      FROM VBRP
      LEFT OUTER JOIN VBRK ON VBRP~VBELN EQ VBRK~VBELN
*        LEFT OUTER JOIN SER01 ON SER01~LIEF_NR <> @SPACE AND SER01~POSNR <> @SPACE
*        LEFT OUTER JOIN OBJK ON SER01~OBKNR EQ OBJK~OBKNR
*        LEFT OUTER JOIN EQUI ON OBJK~EQUNR EQ EQUI~EQUNR
      LEFT OUTER JOIN TVGRT ON VBRP~VKGRP = TVGRT~VKGRP AND TVGRT~SPRAS = 'E'
      LEFT OUTER JOIN VBPA ON VBRP~VBELN = VBPA~VBELN AND PARVW = 'VE'
      LEFT OUTER JOIN PA0001 ON VBPA~PERNR = PA0001~PERNR "#EC CI_BUFFJOIN
        INTO TABLE @LT_INV
...
*          AND SER01~POSNR = @LT_INV_TMP-POSNR_J
    ...
    IF SY-SUBRC = 0.
      ...
      SELECT VBAK~VBELN,
        ...
        INTO TABLE @DATA(LT_CM_DOC)
        FOR ALL ENTRIES IN @LT_INV
        WHERE ( VBAP~VGBEL = @LT_INV-VBELN AND
                VBAP~VGPOS = @LT_INV-POSNR AND
                VBAP~VGTYP = 'M' )
          AND ( AUART = 'ZR01' OR AUART = 'ZR02' )
          AND VBTYP = 'H'.
      IF SY-SUBRC = 0.
        SORT LT_CM_DOC BY VGBEL VGPOS.
      ENDIF.

      LOOP AT LT_INV ASSIGNING FIELD-SYMBOL(<LFS_INV>)."INTO LS_INV.
        DATA(LV_TABIX) = SY-TABIX.
        READ TABLE LT_INV_TMP WITH KEY VBELN = <LFS_INV>-VBELN
                                       POSNR = <LFS_INV>-POSNR
                                       BINARY SEARCH
        ...
        "Case return doc (CM) Is appear
        READ TABLE LT_CM_DOC INTO DATA(LS_CM_DOC)
                             WITH KEY VGBEL = <LFS_INV>-VBELN
                                      VGPOS = <LFS_INV>-POSNR
                            BINARY SEARCH.
        IF SY-SUBRC = 0.
          IF LS_CM_DOC-KWMENG = <LFS_INV>-FKIMG.
            DELETE LT_INV INDEX LV_TABIX.
            CONTINUE.
          ELSE.
            <LFS_INV>-FKIMG = <LFS_INV>-FKIMG - LS_CM_DOC-KWMENG.
          ENDIF.
        ENDIF.
      ENDLOOP.
    ENDIF.
...
*Check if inv no already created warranty letter
    ...
    CLEAR: GT_INV_CRE.
    LOOP AT LT_INV ASSIGNING <LFS_INV>.
      CLEAR: LS_INV_CRE.
...
FORM GET_WARR_CONDTEXT_DF .
  ...
  IF GV_LANG_TH_EN IS NOT INITIAL.
    SELECT * INTO TABLE GT_ZSDSCMC010
      FROM ZSDSCMC010
      WHERE WL_ID = GV_WL_ID
        AND SPRAS = GV_LANG_TH_EN.
    IF SY-SUBRC <> 0.
      SELECT * INTO TABLE GT_ZSDSCMC010
        FROM ZSDSCMC010
        WHERE WL_ID = SPACE
...
FORM DISPLAY_WARR_CONDTEXT_DF .
  ...
  CLEAR: GS_ZSDSCMC010.
  IF GV_LANG_TH_EN IS NOT INITIAL.
    READ TABLE GT_ZSDSCMC010 ASSIGNING FIELD-SYMBOL(<LFS_ZSDSCMC010>) WITH KEY SPRAS = GV_LANG_TH_EN. "#EC CI_NOORDER
  ELSE.
    READ TABLE GT_ZSDSCMC010 ASSIGNING <LFS_ZSDSCMC010> INDEX 1. "#EC CI_NOORDER
  ENDIF.
  IF SY-SUBRC = 0.
    GV_LANG_TH_EN = <LFS_ZSDSCMC010>-SPRAS.
    GS_ZSDSCMC010-SPRAS        = <LFS_ZSDSCMC010>-SPRAS.
    GS_ZSDSCMC010-HEADERTEXT1  = <LFS_ZSDSCMC010>-HEADERTEXT1.
    GS_ZSDSCMC010-HEADERTEXT2  = <LFS_ZSDSCMC010>-HEADERTEXT2.
    GS_ZSDSCMC010-FOOTERTEXT1  = <LFS_ZSDSCMC010>-FOOTERTEXT1.
    GS_ZSDSCMC010-FOOTERTEXT2  = <LFS_ZSDSCMC010>-FOOTERTEXT2.
    GS_ZSDSCMC010-FOOTERTEXT3  = <LFS_ZSDSCMC010>-FOOTERTEXT3.
    GS_ZSDSCMC010-FOOTERTEXT4  = <LFS_ZSDSCMC010>-FOOTERTEXT4.
    GS_ZSDSCMC010-FOOTERTEXT5  = <LFS_ZSDSCMC010>-FOOTERTEXT5.
    GS_ZSDSCMC010-FOOTERTEXT6  = <LFS_ZSDSCMC010>-FOOTERTEXT6.
    GS_ZSDSCMC010-FOOTERTEXT7  = <LFS_ZSDSCMC010>-FOOTERTEXT7.
    GS_ZSDSCMC010-FOOTERTEXT8  = <LFS_ZSDSCMC010>-FOOTERTEXT8.
    GS_ZSDSCMC010-FOOTERTEXT9  = <LFS_ZSDSCMC010>-FOOTERTEXT9.
    GS_ZSDSCMC010-FOOTERTEXT10 = <LFS_ZSDSCMC010>-FOOTERTEXT10.
    GS_ZSDSCMC010-FOOTERTEXT11 = <LFS_ZSDSCMC010>-FOOTERTEXT11.
    GS_ZSDSCMC010-FOOTERTEXT12 = <LFS_ZSDSCMC010>-FOOTERTEXT12.
    GS_ZSDSCMC010-FOOTERTEXT13 = <LFS_ZSDSCMC010>-FOOTERTEXT13.
    GS_ZSDSCMC010-FOOTERTEXT14 = <LFS_ZSDSCMC010>-FOOTERTEXT14.
    GS_ZSDSCMC010-FOOTERTEXT15 = <LFS_ZSDSCMC010>-FOOTERTEXT15.
    GS_ZSDSCMC010-FOOTERTEXT16 = <LFS_ZSDSCMC010>-FOOTERTEXT16.
    GS_ZSDSCMC010-FOOTERTEXT17 = <LFS_ZSDSCMC010>-FOOTERTEXT17.
    GS_ZSDSCMC010-FOOTERTEXT18 = <LFS_ZSDSCMC010>-FOOTERTEXT18.
    GS_ZSDSCMC010-FOOTERTEXT19 = <LFS_ZSDSCMC010>-FOOTERTEXT19.
    GS_ZSDSCMC010-FOOTERTEXT20 = <LFS_ZSDSCMC010>-FOOTERTEXT20.
    GS_ZSDSCMC010-SUBJECT      = <LFS_ZSDSCMC010>-SUBJECT.
    GS_ZSDSCMC010-ATTENTION    = <LFS_ZSDSCMC010>-ATTENTION.
  ENDIF.
ENDFORM.
*&---------------------------------------------------------------------*
...
FORM SAVE_9100_WARR_CONDTEXT_DF .
  ...
  GS_ZSDSCMC010-SPRAS = GV_LANG_TH_EN.

  READ TABLE GT_ZSDSCMC010 ASSIGNING FIELD-SYMBOL(<LFS_C010_TMP>) WITH KEY SPRAS = GS_ZSDSCMC010-SPRAS.
  IF SY-SUBRC = 0.
    <LFS_C010_TMP> = GS_ZSDSCMC010.
    GS_ZSDSCMC010-ZUPD_DATE = SY-DATUM.
    GS_ZSDSCMC010-ZUPD_TIME = SY-UZEIT.
    GS_ZSDSCMC010-ZUPD_USER = SY-UNAME.
  ELSE.
    GS_ZSDSCMC010-ZCRT_DATE = SY-DATUM.
    GS_ZSDSCMC010-ZCRT_TIME = SY-UZEIT.
    GS_ZSDSCMC010-ZCRT_USER = SY-UNAME.
    APPEND GS_ZSDSCMC010 TO GT_ZSDSCMC010.
  ENDIF.

  CALL FUNCTION 'ENQUEUE_EZSDSCMC010'
    EXPORTING
      MODE_ZSDSCMC010 = 'E'
      MANDT           = SY-MANDT
*     SPRAS           =
...
*     _COLLECT        = ' '
  ...
  MODIFY ZSDSCMC010 FROM GS_ZSDSCMC010.

  CALL FUNCTION 'DEQUEUE_EZSDSCMC010'
    EXPORTING
      MODE_ZSDSCMC010 = 'E'
      MANDT           = SY-MANDT
*     SPRAS           =
...
*& text
*&---------------------------------------------------------------------*
FORM SAVE_9300_WARR_CONDTEXT_DF .

  GS_ZSDSCMC010-SPRAS = GV_LANG_TH_EN.
  GS_ZSDSCMC010-WL_ID = GV_WL_ID.
...
FORM VALIDATE_GENC .
  ...
  ZCL_SDSCA_UTILITIES=>GET_GEN_C_RANGE( EXPORTING IF_REPID = SY-CPROG
                                                  IF_PARAM = GC_ALLOW_USER
                                        IMPORTING ET_RANGE = GRT_ALLOW_USR ).
...
FORM PREPARE_DATA_PRINT_FORM .
  ...
  SELECT SINGLE * INTO GS_ZSDSCMC010          "#EC CI_ALL_FIELDS_NEEDED
        FROM ZSDSCMC010
       WHERE  WL_ID = GV_WL_ID
         AND  SPRAS = GV_LANG_TH_EN.
  IF SY-SUBRC <> 0.
  ...
  "Check blank record
  IF GS_ZSDSCMC010-FOOTERTEXT19 IS NOT INITIAL AND
     GS_ZSDSCMC010-FOOTERTEXT20 IS NOT INITIAL.
    DO 18 TIMES.
      LV_RUNNING = SY-INDEX.
      LV_FIELDNAME = 'GS_ZSDSCMC010-FOOTERTEXT' && LV_RUNNING.
      ASSIGN (LV_FIELDNAME) TO FIELD-SYMBOL(<LFS_FIELDNAME>).
      IF <LFS_FIELDNAME> IS ASSIGNED.
        IF <LFS_FIELDNAME> IS NOT INITIAL.
          CONTINUE.
        ELSE.
          "blanks. (do replace)
          <LFS_FIELDNAME> = GS_ZSDSCMC010-FOOTERTEXT19.

          "Do next record
          UNASSIGN <LFS_FIELDNAME>.
          CLEAR LV_FIELDNAME.
          LV_RUNNING = LV_RUNNING + 1.
          LV_FIELDNAME = 'GS_ZSDSCMC010-FOOTERTEXT' && LV_RUNNING.
          ASSIGN (LV_FIELDNAME) TO <LFS_FIELDNAME>.
          IF <LFS_FIELDNAME> IS ASSIGNED.
            <LFS_FIELDNAME> = GS_ZSDSCMC010-FOOTERTEXT20.
          ENDIF.

          CLEAR: GS_ZSDSCMC010-FOOTERTEXT19,
                 GS_ZSDSCMC010-FOOTERTEXT20.
...
*-End of INS by Jutamas Y.
  DATA: LT_MATNR TYPE STANDARD TABLE OF LTYP_MATNR,
        LV_FKIMG TYPE VBRP-FKIMG.
  CHECK GT_INV_100[] IS NOT INITIAL.
  DATA: LT_SERNR TYPE STANDARD TABLE OF OBJK-SERNR.

  PV_INV_PRINT = GV_INV_PRINT.
  PV_LOGO      = GV_LOGO.

*--------------- Set Items ------------------
  SELECT A~VBELN,POSNR,MATNR,FKIMG,A~VGBEL,VGPOS,PRODH,B~BUKRS,B~ZUONR INTO TABLE @DATA(LT_INV) "#EC CI_ALL_FIELDS_NEEDED
  ...
  IF SY-SUBRC = 0.
    ...
    IF SY-SUBRC = 0.
      ...
      SELECT VBFA~VBELV,    "CM Doc
             VBFA~POSNV,
             VBFA~VBELN,    "Do (return)
             VBFA~POSNN,
             OBJK~SERNR
        FROM VBFA
        INNER JOIN SER01 ON ( SER01~LIEF_NR = VBFA~VBELN AND
                              SER01~POSNR   = VBFA~POSNN )
        INNER JOIN OBJK  ON OBJK~OBKNR = SER01~OBKNR
        INTO TABLE @DATA(LT_RETURN_SERNR)
        FOR ALL ENTRIES IN @LT_CM_DOC[]
        WHERE VBFA~VBTYP_N = 'T' "Returns Delivery for Order
          AND ( VBFA~VBELV = @LT_CM_DOC-VBELN AND
                VBFA~POSNV = @LT_CM_DOC-POSNR ).
      IF SY-SUBRC = 0.
        SORT LT_RETURN_SERNR BY SERNR.
      ENDIF.
    ...
    SORT LT_INV BY MATNR FKIMG VGBEL VGPOS.
    DELETE ADJACENT DUPLICATES FROM LT_INV COMPARING MATNR FKIMG VGBEL VGPOS.

    MOVE-CORRESPONDING LT_INV[] TO LT_MATNR[].
    SORT LT_MATNR[] ASCENDING BY MATNR.
    DELETE ADJACENT DUPLICATES FROM LT_MATNR COMPARING MATNR.
    LOOP AT LT_MATNR INTO DATA(LS_MATNR).
      CLEAR : LV_FKIMG,
              LV_SERNR.
      CLEAR: LT_SERNR[].
      LOOP AT LT_INV ASSIGNING FIELD-SYMBOL(<LFS_INV>) WHERE MATNR = LS_MATNR-MATNR.
        "Case return doc (CM) Is appear
        READ TABLE LT_CM_DOC INTO DATA(LS_CM_DOC)
                             WITH KEY VGBEL = <LFS_INV>-VBELN
                                      VGPOS = <LFS_INV>-POSNR
        ...
        LOOP AT LT_SER01 ASSIGNING FIELD-SYMBOL(<LFS_SER01>) WHERE LIEF_NR = <LFS_INV>-VGBEL
          ...
          LOOP AT LT_OBJK ASSIGNING FIELD-SYMBOL(<LFS_OBJK>) WHERE OBKNR = <LFS_SER01>-OBKNR.
            ...
            READ TABLE LT_RETURN_SERNR TRANSPORTING NO FIELDS
                                       WITH KEY SERNR = <LFS_OBJK>-SERNR
                                       BINARY SEARCH.
            IF SY-SUBRC = 0.
              CONTINUE.
            ENDIF.
...
*                SEPARATED BY ', ' .
*            ENDIF.
            APPEND <LFS_OBJK>-SERNR TO LT_SERNR.    "Keep to internal table
          ENDLOOP.
        ENDLOOP.
        ...
        LV_FKIMG = LV_FKIMG + <LFS_INV>-FKIMG.

      ENDLOOP.

*-Beg of INS by Jutamas Y.
      CLEAR LF_FKIMG .
      LF_FKIMG = CEIL( LV_FKIMG ) .
      SHIFT LF_FKIMG LEFT DELETING LEADING SPACE.
*-End of INS by Jutamas Y.
      DATA(LV_LINES) = LINES( LT_SERNR ).
      DATA: LV_INDEX TYPE I VALUE 1.
      IF LV_LINES > 81.
        WHILE LV_INDEX <= LV_LINES.
          CLEAR LV_SERNR.
          LOOP AT LT_SERNR INTO DATA(LS_SERNR)
                           FROM LV_INDEX TO LV_INDEX + 80. " 81 records (index เริ่มจาก 1)
            IF LV_SERNR IS INITIAL.
              LV_SERNR = LS_SERNR.
            ELSE.
              CONCATENATE LV_SERNR LS_SERNR INTO LV_SERNR
                SEPARATED BY ', ' .
            ENDIF.
          ENDLOOP.
...
*-420000361 End of INS
    ...
    CONCATENATE GS_ZSDSCMC010-HEADERTEXT1 GS_ZSDSCMC010-HEADERTEXT2
           INTO LS_HEADER-ACCORDING SEPARATED BY SPACE.

*-420000361 Beg of INS
  ELSE.
    LS_HEADER-ACCORDING =  GS_ZSDSCMC010-HEADERTEXT1.
    LS_HEADER-ACCORDING2 = GS_ZSDSCMC010-HEADERTEXT2.
  ENDIF.
*-420000361 End of INS
...
*----------------- Warranty Condition--------------------
  IF GS_ZSDSCMC010-FOOTERTEXT1 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT1 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT2 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT2 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT3 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT3 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT4 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT4 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT5 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT5 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT6 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT6 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT7 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT7 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT8 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT8 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT9 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT9 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT10 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT10 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT11 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT11 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT12 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT12 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT13 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT13 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT14 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT14 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT15 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT15 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT16 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT16 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT17 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT17 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT18 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT18 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT19 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT19 ) TO PT_COND.
  ENDIF.
  IF GS_ZSDSCMC010-FOOTERTEXT20 IS NOT INITIAL.
    APPEND VALUE #( LINE = GS_ZSDSCMC010-FOOTERTEXT20 ) TO PT_COND.
  ENDIF.
...
*&---------------------------------------------------------------------*
FORM GET_WARR_CONDTEXT_DF_DISPLAY .
  IF SY-UNAME IN GRT_ALLOW_USR[].
    SELECT SINGLE * INTO GS_ZSDSCMC010     ##WARN_OK "#EC CI_NOORDER   "#EC WARNOK  "#EC CI_NOWHERE
      FROM ZSDSCMC010
      WHERE WL_ID = SPACE.
    IF SY-SUBRC = 0.
      GV_LANG_TH_EN = GS_ZSDSCMC010-SPRAS.
    ENDIF.
  ELSE.
    "Remove
    MESSAGE TEXT-E14 TYPE 'E' DISPLAY LIKE 'W'.
  ENDIF.
ENDFORM.
*&---------------------------------------------------------------------*
*& Form GET_WARR_CONDTEXT_DF_DISPLAY
...
*& text
*&---------------------------------------------------------------------*
FORM GET_WARR_CONDTEXT_SPECIFIC.
...
*       _COLLECT        = ' '
    ...
    GS_ZSDSCMT005-STATUS = '1'.
    MODIFY ZSDSCMT005 FROM GS_ZSDSCMT005.
    MESSAGE 'Warranty Letter Saved'(S03) TYPE 'S'.
    PERFORM MANUPULATE_DATA_100.

    CALL FUNCTION 'DEQUEUE_EZSDSCMT005'
      EXPORTING
        MODE_ZSDSCMT005 = 'E'
        MANDT           = SY-MANDT
*       WL_ID           =
